 ...
 * by &f_val structures (that can be compared by val_compare()). In each struct list_head of tree, 
...
struct f_tree *find_tree(struct f_tree *t, struct f_val val)
{
	if (!t)
		return NULL;
	if ((val_compare(t->from, val) != 1) && (val_compare(t->to, val) != -1))
		return t;
...
static struct f_tree *build_tree_rec(struct f_tree **buf, int l, int h)
{
	struct f_tree *n;
	int pos;
...
static int tree_compare(const void *p1, const void *p2)
{
...
static void tree_node_format(struct f_tree *t, struct buffer *buf)
{
	if (t == NULL)
		return;
