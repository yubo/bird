...
{
	return ipa_build6(ntohl(a.s6_addr32[0]), ntohl(a.s6_addr32[1]),
...
static inline struct in6_addr ipa_to_in6(ip_addr a)
{
	return (struct in6_addr) {
		.s6_addr32 = {
	htonl(_I0(a)), htonl(_I1(a)), htonl(_I2(a)),
			    htonl(_I3(a))}};
}
#else
/* Temporary dummy */
...
void sockaddr_fill(struct sockaddr_bird *sa, int af, ip_addr a,
		   struct iface *ifa, uint port);
int sockaddr_read(struct sockaddr_bird *sa, int af, ip_addr * a,
		  struct iface **ifa, uint * port);
