...
void fit_put_next(struct fib *f, struct fib_iterator *i, struct fib_node *n,
		  uint hpos);
...
struct rtable {
	...
	struct config *deleted;	/* Table doesn't exist in current configuration,
				 ...
				 */
	struct event *rt_event;	/* Routing table struct event */
...
struct hostentry {
	...
	struct rta *src;	/* Source struct rta entry */
...
#define REF_COW		1	/* Copy this struct rte on write */
...
#define RTD_ROUTER 0		/* Next hop is struct neighbor router */
...
#define RTAF_CACHED 1		/* This is a cached struct rta */
...
#define EALF_CACHED 4		/* Attributes belonging to cached struct rta */
...
struct eattr *ea_find(struct ea_list *, unsigned ea);
struct eattr *ea_walk(struct ea_walk_state *s, uint id, uint max);
int ea_get_int(struct ea_list *, unsigned ea, int def);
 ...
 * rta->hostentry field.  New hostentry has zero use count. Cached struct rta locks its
 * hostentry (increases its use count), uncached struct rta does not lock it. Hostentry
...
void roa_add_item(struct roa_table *t, ip_addr prefix, byte pxlen, byte maxlen,
		  u32 asn, byte src);
void roa_delete_item(struct roa_table *t, ip_addr prefix, byte pxlen,
		     byte maxlen, u32 asn, byte src);
...
void roa_add_item_config(struct roa_table_config *rtc, ip_addr prefix,
			 byte pxlen, byte maxlen, u32 asn);
void roa_init(void);
