...
static inline void rt_schedule_gc(struct rtable *tab);
static inline void rt_schedule_prune(struct rtable *tab);

static inline struct ea_list *make_tmp_attrs(struct rte *rt,
					     struct linpool *pool)
{
 ...
 * the protocol.
 */
struct rte *rte_get_temp(struct rta *a)
{
	struct rte *e = sl_alloc(rte_slab);
...
struct rte *rte_do_cow(struct rte *r)
{
	struct rte *e = sl_alloc(rte_slab);

	memcpy(e, r, sizeof(struct rte));
 ...
 * @lp: a struct linpool from which to allocate &rta
 *
...
struct rte *rte_cow_rta(struct rte *r, struct linpool *lp)
{
	...
	struct rte *e = rte_cow(r);
	struct rta *a = rta_do_cow(r->attrs, lp);
	rta_free(e->attrs);
	e->attrs = a;
	return e;
}

static int /* Actually better or at least as good as */ rte_better(struct rte
								   *new,
								   struct rte
								   *old)
{
	int (*better) (struct rte *, struct rte *);
	...
	if (new->attrs->src->proto->proto != old->attrs->src->proto->proto) {
		 ...
		 */
		return new->attrs->src->proto->proto >
		    old->attrs->src->proto->proto;
	}
...
static int rte_mergable(struct rte *pri, struct rte *sec)
{
...
static void rte_trace(struct proto *p, struct rte *e, int dir, char *msg)
{
	byte via[STD_ADDRESS_P_LENGTH + 32];

	rt_format_via(e, via);
	log(L_TRACE "%s %c %s %I/%d %s", p->name, dir, msg, e->net->n.prefix,
	    e->net->n.pxlen, via);
}

static inline void
rte_trace_in(uint flag, struct proto *p, struct rte *e, char *msg)
{
	if (p->debug & flag)
		rte_trace(p, e, '>', msg);
}
...
static inline void
rte_trace_out(uint flag, struct proto *p, struct rte *e, char *msg)
{
	if (p->debug & flag)
		rte_trace(p, e, '<', msg);
}

static struct rte *export_filter(struct announce_hook *ah, struct rte *rt0,
				 ...
				 int silent)
{
	...
	*tmpa = make_tmp_attrs(rt, rte_update_pool);
	...
	v = p->import_control ? p->import_control(p, &rt, tmpa,
						  rte_update_pool) : 0;
	if (v < 0) {
		if (silent)
			goto reject;
	...
	v = filter && ((filter == FILTER_REJECT) ||
		       (f_run
			(filter, &rt, tmpa, rte_update_pool,
			 FF_FORCE_TMPATTR) > F_ACCEPT));
	if (v) {
		if (silent)
			goto reject;
		...
		struct rte *old0, int refeed)
{
	...
	struct rte *new = new0;
	struct rte *old = old0;
	struct rte *new_free = NULL;
	struct rte *old_free = NULL;
		   ...
		   struct rte *old_changed, struct rte *before_old, int feed)
{
	...
	struct rte *r;
	struct rte *new_best = NULL;
	struct rte *old_best = NULL;
	struct rte *new_free = NULL;
	struct rte *old_free = NULL;
	struct ea_list *tmpa = NULL;
	...
	if (old_meet)
		if (old_best =
		    export_filter(ah, old_changed, &old_free, NULL, 1))
			goto found;
			     ...
			     int silent)
{
	...
	struct rte *best0, *best, *rt0, *rt, *tmp;
		 ...
		 struct rte *old_best, int refeed)
{
	...
	struct rte *new_best_free = NULL;
	struct rte *old_best_free = NULL;
	struct rte *new_changed_free = NULL;
	struct rte *old_changed_free = NULL;
	struct ea_list *tmpa = NULL;

	/* We assume that all struct rte arguments are either NULL or rte_is_valid() */
	...
	if ((new_best == old_best) && !refeed) {
		new_changed = rte_mergable(new_best, new_changed) ?
		    export_filter(ah, new_changed, &new_changed_free, NULL,
				  1) : NULL;

		old_changed = rte_mergable(old_best, old_changed) ?
		    export_filter(ah, old_changed, &old_changed_free, NULL,
				  1) : NULL;
	     ...
	     struct rte *before_old)
{
	...
	list_for_each_entry(a, &tab->hooks, n) {
		...
		if (a->proto->accept_ra_types == type)
			if (type == RA_ACCEPTED)
				rt_notify_accepted(a, net, new, old, before_old,
						   0);
			else if (type == RA_MERGED)
				rt_notify_merged(a, net, new, old, new_best,
						 old_best, 0);
			else
...
static int rte_same(struct rte *x, struct rte *y)
{
	return
	    x->attrs == y->attrs &&
	    x->flags == y->flags &&
	    x->pflags == y->pflags &&
	    x->pref == y->pref &&
	    (!x->attrs->src->proto->rte_same
	     || x->attrs->src->proto->rte_same(x, y));
}

static inline int rte_is_ok(struct rte *e)
{
	return e && !rte_is_filtered(e);
}
		...
		struct rte_src *src)
{
	...
	struct rte *before_old = NULL;
	struct rte *old_best = net->routes;
	struct rte *old = NULL;
	struct rte **k;
...
skip_stats1:
	...
	if (new)
		rte_is_filtered(new) ? stats->filt_routes++ : stats->
		    imp_routes++;
	if (old)
		rte_is_filtered(old) ? stats->filt_routes-- : stats->
		    imp_routes--;
	...
	if (table->config->sorted) {
		...
		if (src->proto->rte_recalculate
		    && src->proto->rte_recalculate(table, net, new, old,
						   old_best))
			goto do_recalculate;
...
do_recalculate:
	...
	if (net->routes != old_best)
		rte_announce(table, RA_OPTIMAL, net, net->routes, old_best,
			     NULL, NULL, NULL);
	if (table->config->sorted)
		rte_announce(table, RA_ACCEPTED, net, new, old, NULL, NULL,
			     before_old);
	rte_announce(table, RA_MERGED, net, new, old, net->routes, old_best,
		     NULL);
 ...
 * from a special linear struct pool @rte_update_pool and freed when rte_update()
 * finishes.
 */
...
void rte_discard(struct rtable *t, struct rte *old)
	   ...
	   struct filter *filter)
{
	...
	struct ea_list *tmpa = make_tmp_attrs(rt, rte_update_pool);
	int v =
	    p->import_control ? p->import_control(p, &rt, &tmpa,
						  rte_update_pool) : 0;
	if (v == RIC_PROCESS)
		v = (f_run
		     (filter, &rt, &tmpa, rte_update_pool,
...
static inline void rt_schedule_hcu(struct rtable *tab)
{
...
static inline void rt_schedule_nhu(struct rtable *tab)
{
	if (tab->nhu_state == 0)
		ev_schedule(tab->rt_event);
...
static void rt_prune_nets(struct rtable *tab)
{
...
static void rt_event(void *ptr)
{
	struct rtable *tab = ptr;
...
static inline int rta_next_hop_outdated(struct rta *a)
{
...
static inline void rta_apply_hostentry(struct rta *a, struct hostentry *he)
{
	a->hostentry = he;
...
static inline struct rte *rt_next_hop_update_rte(struct rtable *tab,
						 struct rte *old)
{
	struct rta a;
	memcpy(&a, old->attrs, sizeof(struct rta));
	rta_apply_hostentry(&a, old->attrs->hostentry);
	a.aflags = 0;

	struct rte *e = sl_alloc(rte_slab);
	memcpy(e, old, sizeof(struct rte));
	...
	for (k = &n->routes; e = *k; k = &e->next)
		if (rta_next_hop_outdated(e->attrs)) {
			...
			*k = new;
			...
			rte_trace_in(D_ROUTES, new->sender->proto, new,
				     "updated");

			/* Call a pre-comparison hook */
			/* Not really an efficient way to compute this */
			if (e->attrs->src->proto->rte_recalculate)
				e->attrs->src->proto->rte_recalculate(tab, n,
								      new, e,
								      NULL);
 ...
 * configuration.
 */
void rt_lock_table(struct rtable *r)
{
	r->use_count++;
}
 ...
 */
void rt_unlock_table(struct rtable *r)
{
			...
			if (!ot->deleted) {
				...
				if (sym && sym->class == SYM_TABLE
					...
					if (o->sorted != r->sorted)
						log(L_WARN
						    "Reconfiguration of struct rtable sorted flag not implemented");
				} else {
					DBG("\t%s: deleted\n", o->name);
					ot->deleted = old;
	     ...
	     struct rte *e)
{
	...
	if (type == RA_ACCEPTED)
		...
		rt_notify_merged(h, n, NULL, NULL, e, p->refeeding ? e : NULL,
				 p->refeeding);
	else
...
int rt_feed_baby(struct proto *p)
{
	...
	if (!p->feed_ahook) {	/* Need to initialize first */
		...
		fit = p->feed_iterator =
		    mb_alloc(p->pool, sizeof(struct fib_iterator));
		goto next_hook;
	}
	fit = p->feed_iterator;
...
static inline unsigned ptr_hash(void *ptr)
{
	uintptr_t p = (uintptr_t) ptr;
	return p ^ (p << 8) ^ (p >> 16);
}

static inline unsigned hc_hash(ip_addr a, struct rtable *dep)
{
	return (ipa_hash(a) ^ ptr_hash(dep)) & 0xffff;
}

static inline void hc_insert(struct hostcache *hc, struct hostentry *he)
{
...
static inline void hc_remove(struct hostcache *hc, struct hostentry *he)
{
...
static void hc_alloc_table(struct hostcache *hc, unsigned order)
{
	...
	hc->hash_table =
	    mb_allocz(rt_table_pool, hsize * sizeof(struct hostentry *));
}

static void hc_resize(struct hostcache *hc, unsigned new_order)
{
...
static struct hostentry *hc_new_hostentry(struct hostcache *hc, ip_addr a,
					  ip_addr ll, struct rtable *dep,
					  unsigned k)
{
...
static void hc_delete_hostentry(struct hostcache *hc, struct hostentry *he)
{
	rta_free(he->src);
...
static void rt_init_hostcache(struct rtable *tab)
{
	struct hostcache *hc =
...
static void rt_free_hostcache(struct rtable *tab)
{
	...
	list_for_each_entry(he, &hc->hostentries, ln) {
		...
		if (he->uc)
			log(L_ERR "Hostcache is not empty in table %s",
			    tab->name);
	}
...
static int if_local_addr(ip_addr a, struct iface *i)
{
	struct ifa *b;
...
static u32 rt_get_igp_metric(struct rte *rt)
{
	struct eattr *ea = ea_find(rt->attrs->eattrs, EA_GEN_IGP_METRIC);

	if (ea)
		return ea->u.data;

	struct rta *a = rt->attrs;

#ifdef CONFIG_OSPF
	if ((a->source == RTS_OSPF) ||
	    (a->source == RTS_OSPF_IA) || (a->source == RTS_OSPF_EXT1))
...
static int rt_update_hostentry(struct rtable *tab, struct hostentry *he)
{
	struct rta *old_src = he->src;
	int pxlen = 0;
	...
	trie_add_prefix(tab->hostcache->trie, he->addr, MAX_PREFIX_LENGTH,
...
static struct hostentry *rt_get_hostentry(struct rtable *tab, ip_addr a,
					  ip_addr ll, struct rtable *dep)
{
	struct hostentry *he;
	...
	struct hostcache *hc = tab->hostcache;
	for (he = hc->hash_table[k >> hc->hash_shift]; he != NULL;
	     he = he->next)
...
void
rta_set_recursive_next_hop(struct rtable *dep, struct rta *a,
...
static void rt_format_via(struct rte *e, byte * via)
{
	...
	switch (a->dest) {
	case RTD_ROUTER:
		bsprintf(via, "via %I on %s", a->gw, a->iface->name);
		break;
	case RTD_DEVICE:
		bsprintf(via, "dev %s", a->iface->name);
		break;
	case RTD_BLACKHOLE:
...
static void
rt_show_rte(struct cli *c, byte * ia, struct rte *e, struct rt_show_data *d,
	    struct ea_list *tmpa)
{
	...
	cli_printf(c, -1007, "%-18s %s [%s %s%s]%s%s", ia, via,
		   a->src->proto->name, tm, from,
		   primary ? (sync_error ? " !" : " *") : "", info);
	for (nh = a->nexthops; nh; nh = nh->next)
		cli_printf(c, -1007, "\tvia %I on %s weight %d", nh->gw,
			   nh->iface->name, nh->weight + 1);
	if (d->verbose)
		rta_show(c, a, tmpa);
}
...
static void rt_show_net(struct cli *c, struct network * n, struct rt_show_data *d)
{
	...
	for (e = n->routes; e; e = e->next) {
		...
		ee = e;
		rte_update_lock();	/* We use the update struct buffer for filtering */
		tmpa = make_tmp_attrs(e, rte_update_pool);

		/* Special case for merged export */
		if ((d->export_mode == RSEM_EXPORT)
		    && (d->export_protocol->accept_ra_types == RA_MERGED)) {
			struct rte *rt_free;
			...
			int ic =
			    ep->import_control ? ep->import_control(ep, &e,
								    &tmpa,
								    rte_update_pool)
			    : 0;

			if (ep->accept_ra_types == RA_OPTIMAL
			    || ep->accept_ra_types == RA_MERGED)
				pass = 1;
			...
			if (d->export_mode > RSEM_PREEXPORT) {
				...
				int do_export = (ic > 0) ||
				    (f_run
				     (a->out_filter, &e, &tmpa, rte_update_pool,
				      FF_FORCE_TMPATTR) <= F_ACCEPT);

				if (do_export !=
				    (d->export_mode == RSEM_EXPORT))
					goto skip;

				if ((d->export_mode == RSEM_EXPORT)
				    && (ep->accept_ra_types == RA_ACCEPTED))
					pass = 1;
			}
		}

		if (d->show_protocol
		    && (d->show_protocol != e->attrs->src->proto))
			goto skip;

		if (f_run
		    (d->filter, &e, &tmpa, rte_update_pool,
		     FF_FORCE_TMPATTR) > F_ACCEPT)
			goto skip;
...
static void rt_show_cont(struct cli *c)
{
	...
	FIB_ITERATE_START(fib, it, f) {
		...
		if (d->export_protocol
		    && (d->export_protocol->export_state == ES_DOWN)) {
	...
	FIB_ITERATE_END(f);
	if (d->stats)
		cli_printf(c, 14, "%d of %d routes for %d networks",
			   d->show_counter, d->rt_counter, d->net_counter);
	else
		cli_printf(c, 0, "");
done:
...
static void rt_show_cleanup(struct cli *c)
{
	struct rt_show_data *d = c->rover;
...
void rt_show(struct rt_show_data *d)
{
	...
	if (!d->table && d->export_protocol)
		d->table = d->export_protocol->table;
	if (!d->table && d->show_protocol)
		d->table = d->show_protocol->table;
	if (!d->table)
