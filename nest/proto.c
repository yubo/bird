...
static struct timer *proto_shutdown_timer;
static struct timer *gr_wait_timer;
 ...
 */
void *proto_new(struct proto_config *c, unsigned size)
{
...
static void proto_init_instance(struct proto *p)
{
 ...
 * allocated from the routing table struct resource struct pool and when protocol accepts
 ...
 * code.
 */
struct announce_hook *proto_add_announce_hook(struct proto *p, struct rtable *t,
 ...
 */
struct announce_hook *proto_find_announce_hook(struct proto *p,
					       struct rtable *t)
{
...
static void proto_link_ahooks(struct proto *p)
{
	struct announce_hook *h;
...
void proto_copy_config(struct proto_config *dest, struct proto_config *src)
{
	...
	if (dest->protocol != src->protocol)
		cf_error
		    ("Can't copy configuration from a different protocol type");
 ...
 */
void protos_postconfig(struct config *c)
{
...
static struct proto *proto_init(struct proto_config *c)
{
	      ...
	      int type)
{
	...
	if (old) {
		list_for_each_entry(oc, &old->protos, n) {
			...
			if (sym && sym->class == SYM_PROTO && !new->shutdown) {
				...
				if (!force_reconfig
				    && proto_reconfigure(p, oc, nc, type))
					continue;

				/* Unsuccessful, we will restart it */
				if (!p->disabled && !nc->disabled)
					log(L_INFO "Restarting protocol %s",
					    p->name);
				else if (p->disabled && !nc->disabled)
					log(L_INFO "Enabling protocol %s",
					    p->name);
				else if (!p->disabled && nc->disabled)
					log(L_INFO "Disabling protocol %s",
					    p->name);

				p->down_code =
				    nc->
				    disabled ? PDC_CF_DISABLE : PDC_CF_RESTART;
				p->cf_new = nc;
	...
	if (!config->router_id) {
		config->router_id =
		    if_choose_router_id(config->router_id_from, 0);
		if (!config->router_id)
 ...
 * locks are unlocked or when graceful restart wait struct timer fires.
 *
 */
 ...
 * restart wait struct timer fires (but there are still some locks).
 */
...
void graceful_restart_show_status(void)
{
	...
	cli_msg(-24, "  Waiting for %d protocols to recover",
		graceful_restart_locks);
	cli_msg(-24, "  Wait struct timer is %d/%d", tm_remains(gr_wait_timer),
		config->gr_wait);
}
...
static void proto_feed_more(void *P)
{
	struct proto *p = P;
...
static void proto_feed_initial(void *P)
{
	struct proto *p = P;
...
static void proto_schedule_feed(struct proto *p, int initial)
{
...
static void proto_schedule_flush_loop(void)
{
	struct proto *p;
	struct announce_hook *h;
...
static void proto_flush_loop(void *unused UNUSED)
{
	struct proto *p;
...
static void proto_shutdown_loop(struct timer *t UNUSED)
{
...
static inline void proto_schedule_down(struct proto *p, byte restart, byte code)
{
...
static const char *proto_limit_name(struct proto_limit *l)
{
	const char *actions[] = {
		[PLA_WARN] = "warn",
		[PLA_BLOCK] = "block",
...
static void proto_want_core_up(struct proto *p)
{
...
static void proto_want_export_down(struct proto *p)
{
	ASSERT(p->export_state != ES_DOWN);
...
static void proto_want_core_down(struct proto *p)
{
	ASSERT(p->core_state == FS_HAPPY);
	ASSERT(p->export_state == ES_DOWN);
...
static void proto_falling_down(struct proto *p)
{
	p->gr_recovery = 0;
	p->gr_wait = 0;
	if (p->gr_lock)
...
static void proto_fell_down(struct proto *p)
{
	...
	if (all_routes != 0)
		log(L_ERR "Protocol %s is down but still has %d routes",
		    p->name, all_routes);
 ...
 * @ps: the new status
 *
 * Whenever a state of a protocol changes due to some struct event internal
 ...
 */
void proto_notify_state(struct proto *p, unsigned ps)
{
	...
	DBG("%s reporting state transition %s/%s -> */%s\n", p->name,
	    c_states[cs], p_states[ops], p_states[ps]);
	if (ops == ps)
		return;
...
static void proto_show_stats(struct proto_stats *s, int in_keep_filtered)
{
	if (in_keep_filtered)
		cli_msg(-1006,
		...
		cli_msg(-1006,
			"  Routes:         %u imported, %u exported, %u preferred",
...
void proto_cmd_show(struct proto *p, uint verbose, int cnt)
{
	...
	cli_msg(-1002, "%-8s %-8s %-8s %-5s  %-10s  %s",
		p->name,
		p->proto->name, p->table->name, proto_state_name(p), tbuf, buf);
	if (verbose) {
		if (p->cf->dsc)
			cli_msg(-1006, "  Description:    %s", p->cf->dsc);
		if (p->cf->router_id)
			cli_msg(-1006, "  Router ID:      %R",
				p->cf->router_id);
...
void proto_cmd_disable(struct proto *p, uint arg UNUSED, int cnt UNUSED)
{
	if (p->disabled) {
...
void proto_cmd_enable(struct proto *p, uint arg UNUSED, int cnt UNUSED)
{
	if (!p->disabled) {
...
void proto_cmd_restart(struct proto *p, uint arg UNUSED, int cnt UNUSED)
{
	if (p->disabled) {
		cli_msg(-8, "%s: already disabled", p->name);
		return;
...
void proto_cmd_reload(struct proto *p, uint dir, int cnt UNUSED)
{
	if (p->disabled) {
		cli_msg(-8, "%s: already disabled", p->name);
		return;
...
void proto_cmd_debug(struct proto *p, uint mask, int cnt UNUSED)
{
	p->debug = mask;
}

void proto_cmd_mrtdump(struct proto *p, uint mask, int cnt UNUSED)
{
	p->mrtdump = mask;
}

static void
proto_apply_cmd_symbol(struct symbol *s,
		       void (*cmd) (struct proto *, uint, int), uint arg)
{
...
struct proto *proto_get_named(struct symbol *sym, struct protocol *pr)
{
	struct proto *p, *q;
