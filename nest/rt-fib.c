...
static void fib_ht_alloc(struct fib *f)
{
	...
	f->hash_table =
	    mb_alloc(f->fib_pool, f->hash_size * sizeof(struct fib_node *));
}

static inline void fib_ht_free(struct fib_node **h)
{
	mb_free(h);
}

static inline unsigned fib_hash(struct fib *f, ip_addr * a)
{
	return ipa_hash(*a) >> f->hash_shift;
}

static void fib_dummy_init(struct fib_node *dummy UNUSED)
{
}
 ...
 */
void
fib_init(struct fib *f, struct pool * p, unsigned node_size, unsigned hash_order,
	 fib_init_func init)
{
	if (!hash_order)
...
struct fib_node *fit_get(struct fib *f, struct fib_iterator *i)
{
	struct fib_node *n;
...
void
fit_put_next(struct fib *f, struct fib_iterator *i, struct fib_node *n,
	     uint hpos)
{
	if (n = n->next)
		goto found;
...
void fib_check(struct fib *f)
{
	...
	for (i = 0; i < f->hash_size; i++) {
		...
		for (n = f->hash_table[i]; n; n = n->next) {
			...
			if ((h0 >> f->hash_shift) != i)
				bug("fib_check: mishashed %x->%x (order %d)",
				    h0, i, f->hash_order);
	...
	if (ec != f->entries)
		bug("fib_check: invalid entry count (%d != %d)", ec,
		    f->entries);
}
...
void dump(char *m)
{
	...
	for (i = 0; i < f.hash_size; i++) {
		...
		struct fib_iterator *j;
		for (n = f.hash_table[i]; n; n = n->next) {
			debug("%04x %04x %p %I/%2d", i, ipa_hash(n->prefix), n,
			      n->prefix, n->pxlen);
			for (j = n->readers; j; j = j->next)
				debug(" %p[%p]", j, j->node);
			debug("\n");
