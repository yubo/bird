 ...
 * those which are always present) as structure members and a struct list_head of
 * dynamic attributes represented by a linked struct list_head of &ea_list
...
static inline int u32_cto(uint x)
{
	return ffs(~x) - 1;
}

static inline u32 rte_src_alloc_id(void)
{
	int i, j;
...
static inline void rte_src_free_id(u32 id)
{
	int i = id / 32;
	int j = id % 32;
...
static inline uint mpnh_hash(struct mpnh *x)
{
	uint h = 0;
	for (; x; x = x->next)
		h ^= ipa_hash(x->gw);
...
int mpnh__same(struct mpnh *x, struct mpnh *y)
{
...
static int mpnh_compare_node(struct mpnh *x, struct mpnh *y)
{
	int r;
...
static inline struct mpnh *mpnh_copy_node(const struct mpnh *src,
					  struct linpool *lp)
{
 ...
 * @x: struct list_head 1
 * @y: struct list_head 2
 * @rx: reusability of struct list_head @x
 * @ry: reusability of struct list_head @y
 ...
 * resulting struct list_head is no longer needed. When reusability is not set, the
...
static struct mpnh *mpnh_copy(struct mpnh *o)
{
...
static void mpnh_free(struct mpnh *o)
{
	struct mpnh *n;
...
static inline struct eattr *ea__find(struct ea_list *e, unsigned id)
{
	struct eattr *a;
	int l, r, m;
 ...
 * @e: attribute struct list_head to search in
 ...
 */
struct eattr *ea_find(struct ea_list *e, unsigned id)
{
 ...
 *
 * Given an extended attribute list, ea_walk() walks through the struct list_head looking
 ...
 * attribute exists. The extended attribute struct list_head or the arguments should not be
 * modified between calls. The maximum value of @max is 128.
 */
struct eattr *ea_walk(struct ea_walk_state *s, uint id, uint max)
{
 ...
 */
int ea_get_int(struct ea_list *e, unsigned id, int def)
{
...
static inline void ea_do_sort(struct ea_list *e)
{
	unsigned n = e->count;
...
static inline void ea_do_prune(struct ea_list *e)
{
 ...
 * @e: struct list_head to be sorted
 *
 ...
 * by calling ea_sort().
 */
void ea_merge(struct ea_list *e, struct ea_list *t)
{
 ...
 */
int ea_same(struct ea_list *x, struct ea_list *y)
{
	int c;
	...
	for (c = 0; c < x->count; c++) {
		...
		if (a->id != b->id ||
		    a->flags != b->flags ||
		    a->type != b->type ||
		    ((a->type & EAF_EMBEDDED) ? a->u.data !=
		     b->u.data : !adata_same(a->u.ptr, b->u.ptr)))
			return 0;
	}
	return 1;
}

static inline struct ea_list *ea_list_copy(struct ea_list *o)
{
	struct ea_list *n;
	unsigned i, len;
...
static inline void ea_free(struct ea_list *o)
{
	int i;
...
static int get_generic_attr(struct eattr *a, byte ** buf, int buflen UNUSED)
{
...
void
ea_format_bitfield(struct eattr *a, byte * buf, int bufsize, const char **names,
		   int min, int max)
{
...
static inline void opaque_format(struct adata *ad, byte * buf, uint size)
{
...
static inline void
ea_show_int_set(struct cli *c, struct adata *ad, int way, byte * pos,
		byte * buf, byte * end)
{
...
static inline void
ea_show_ec_set(struct cli *c, struct adata *ad, byte * pos, byte * buf,
	       byte * end)
{
 ...
 */
void ea_show(struct cli *c, struct eattr *e)
{
	struct protocol *p;
...
void ea_dump(struct ea_list *e)
{
	...
	while (e) {
		...
		for (i = 0; i < e->count; i++) {
			...
			debug(" %02x:%02x.%02x", EA_PROTO(a->id), EA_ID(a->id),
			      a->flags);
			if (a->type & EAF_TEMP)
				debug("T");
			debug("=%c",
			      "?iO?I?P???S?????"[a->type & EAF_TYPE_MASK]);
 ...
 * @e: attribute list
 *
 * ea_hash() takes an extended attribute struct list_head and calculated a hopefully
 * uniformly distributed hash value from its contents.
 */
inline uint ea_hash(struct ea_list *e)
{
	u32 h = 0;
	int i;
 ...
 * @to: destination struct list_head (can be %NULL)
 * @what: struct list_head to be appended (can be %NULL)
 *
...
static void rta_alloc_hash(void)
{
	rta_hash_table =
	    mb_allocz(rta_pool, sizeof(struct rta *) * rta_cache_size);
...
static inline uint rta_hash(struct rta *a)
{
	return (((uint) (uintptr_t) a->src) ^ ipa_hash(a->gw) ^
		mpnh_hash(a->nexthops) ^ ea_hash(a->eattrs)) & 0xffff;
}

static inline int rta_same(struct rta *x, struct rta *y)
{
	return (x->src == y->src &&
		x->source == y->source &&
		x->scope == y->scope &&
...
static struct rta *rta_copy(struct rta *o)
{
...
static inline void rta_insert(struct rta *r)
{
 ...
 */
struct rta *rta_lookup(struct rta *o)
{
	struct rta *r;
	uint h;
...
void rta_dump(struct rta *a)
{
	...
	debug("p=%s uc=%d %s %s%s%s h=%04x",
	      a->src->proto->name, a->uc, rts[a->source],
	      ip_scope_text(a->scope), rtc[a->cast], rtd[a->dest], a->hash_key);
...
void rta_show(struct cli *c, struct rta *a, struct ea_list *eal)
{
	static char *src_names[] =
	    { "dummy", "static", "inherit", "device", "static-device",
"redirect",
		"RIP", "OSPF", "OSPF-IA", "OSPF-E1", "OSPF-E2", "BGP", "pipe"
	};
	static char *cast_names[] =
