...
static void dev_ifa_notify(struct proto *p, unsigned c, struct ifa *ad)
{
	struct rt_dev_config *P = (void *)p->cf;

	if (!list_empty(&P->iface_list) &&
	    !iface_patt_find(&P->iface_list, ad->iface, ad->iface->addr))
		/* Empty struct list_head is automagically treated as "*" */
		return;
...
static void dev_if_notify(struct proto *p, uint c, struct iface *iface)
{
	...
	if ((c & IF_CHANGE_LINK) && cf->check_link) {
		uint ac =
		    (iface->flags & IF_LINK_UP) ? IF_CHANGE_UP : IF_CHANGE_DOWN;
...
static struct proto *dev_init(struct proto_config *c)
{
...
static int dev_reconfigure(struct proto *p, struct proto_config *new)
{
...
static void dev_copy_config(struct proto_config *dest, struct proto_config *src)
{
	 ...
	 */
	cfg_copy_list(&d->iface_list, &s->iface_list,
