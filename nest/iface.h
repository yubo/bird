...
extern struct list_head iface_list;
...
struct iface {
	...
	struct list_head addrs;	/* Addresses assigned to this interface */
	struct ifa *addr;	/* Primary address */
	struct list_head neighbors;	/* All neighbors on this interface */
};
...
struct neighbor {
	struct list_head n;	/* Node in global struct neighbor struct list_head */
	struct list_head if_n;	/* Node in per-interface struct neighbor struct list_head */
...
static inline int neigh_connected_to(struct proto *p, ip_addr * a,
				     struct iface *i)
{
...
struct iface_patt {
	struct list_head n;
	struct list_head ipn_list;	/* A struct list_head of struct iface_patt_node */
...
struct iface_patt *iface_patt_find(struct list_head *l, struct iface *i,
				   struct ifa *a);
int iface_patts_equal(struct list_head *, struct list_head *,
		      int (*)(struct iface_patt *, struct iface_patt *));
