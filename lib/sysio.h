...
#define INIT_MREQ4(maddr,ifa) \
  { .imr_multiaddr = ipa_to_in4(maddr), .imr_ifindex = ifa->index < DPVS_PORT_FLAG ? ifa->index : 0 }

static inline int sk_setup_multicast4(struct birdsock *s)
{
	struct ip_mreqn mr = {.imr_ifindex = s->iface->index < DPVS_PORT_FLAG ? s->iface->index : 0};
	int ttl = s->ttl;
	int n = 0;
		  ...
		  size_t cbuflen)
{
	...
	pi->ipi_ifindex = s->iface ? (s->iface->index < DPVS_PORT_FLAG ? s->iface->index : 0): 0;
