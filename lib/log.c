...
static int main_thread_self(void)
{
 ...
 *
 * This function writes a message prepared in the log struct buffer to the
 * log file (as specified in the configuration). The log struct buffer is
...
static void vlog(int class, const char *msg, va_list args)
{
	struct buffer buf;
	LOG_BUFFER_INIT(buf);
