...
static void lp_free(struct resource *);
static void lp_dump(struct resource *);
static struct resource *lp_lookup(struct resource *, unsigned long);
static size_t lp_memsize(struct resource *r);
...
{
	struct linpool *m = ralloc(p, &lp_class);
	m->plast = &m->first;
	m->chunk_size = blk;
	m->threshold = 3 * blk / 4;
	return m;
 ...
 */
void *lp_alloc(struct linpool *m, uint size)
{
	...
	if (e <= m->end) {
		...
		if (size >= m->threshold) {
			...
			if (m->current) {
				...
				c = xmalloc(sizeof(struct lp_chunk) +
					    m->chunk_size);
 ...
 */
void *lp_allocu(struct linpool *m, uint size)
{
	byte *a = m->ptr;
	byte *e = a + size;
 ...
 */
void *lp_allocz(struct linpool *m, uint size)
{
 ...
 * but leaves the struct pool itself.
 */
void lp_flush(struct linpool *m)
{
	struct lp_chunk *c;
...
static void lp_free(struct resource *r)
{
	struct linpool *m = (struct linpool *)r;
	struct lp_chunk *c, *d;
...
static void lp_dump(struct resource *r)
{
	struct linpool *m = (struct linpool *)r;
	struct lp_chunk *c;
	int cnt, cntl;
	...
	debug("(chunk=%d threshold=%d count=%d+%d total=%d+%d)\n",
	      m->chunk_size, m->threshold, cnt, cntl, m->total, m->total_large);
}
...
static struct resource *lp_lookup(struct resource *r, unsigned long a)
{
	struct linpool *m = (struct linpool *)r;
	struct lp_chunk *c;

	for (c = m->first; c; c = c->next)
		if ((unsigned long)c->data <= a
		    && (unsigned long)c->data + c->size > a)
			return r;
	for (c = m->first_large; c; c = c->next)
		if ((unsigned long)c->data <= a
		    && (unsigned long)c->data + c->size > a)
