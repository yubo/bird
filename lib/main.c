...
void
cli_write_trigger(struct cli *c)
{
  struct birdsock *s = c->priv;
...
static void
cli_tx(struct birdsock *s)
{
  cli_write(s->data);
}

int
cli_get_command(struct cli *c)
{
  struct birdsock *s = c->priv;
...
static int
cli_rx(struct birdsock *s, int size UNUSED)
{
  cli_kick(s->data);
  return 0;
}
...
static void
cli_err(struct birdsock *s, int err)
{
  if (config->cli_debug)
    {
      if (err)
...
static int
cli_connect(struct birdsock *s, int size UNUSED)
{
  struct cli *c;
