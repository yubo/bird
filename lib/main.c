...
static inline void drop_uid(uid_t uid)
{
...
static inline void drop_gid(gid_t gid)
{
	if (setgid(gid) < 0)
		die("setgid: %m");
}
...
static inline void add_num_const(char *name, int val)
{
...
static void read_iproute_table(char *file, char *prefix, int max)
{
...
static int cf_read(byte * dest, uint len, int fd)
{
...
static int unix_read_config(struct config **cp, char *name)
{
...
static struct config *read_config(void)
{
	...
	if (!unix_read_config(&conf, config_name)) {
		if (conf->err_msg)
			die("%s, line %d: %s", conf->err_file_name,
			    conf->err_lino, conf->err_msg);
		else
			die("Unable to open configuration file %s: %m",
			    config_name);
	}
...
void async_config(void)
{
	...
	if (!unix_read_config(&conf, config_name)) {
		if (conf->err_msg)
			log(L_ERR "%s, line %d: %s", conf->err_file_name,
			    conf->err_lino, conf->err_msg);
		else
			log(L_ERR "Unable to open configuration file %s: %m",
			    config_name);
		config_free(conf);
	} else
...
static struct config *cmd_read_config(char *name)
{
	struct config *conf;
	...
	if (!unix_read_config(&conf, name)) {
		if (conf->err_msg)
			cli_msg(8002, "%s, line %d: %s", conf->err_file_name,
				conf->err_lino, conf->err_msg);
		else
...
static void cmd_reconfig_msg(int r)
{
	...
	case CONF_QUEUED:
		cli_msg(5,
			"Reconfiguration already in progress, queueing new config");
		break;
	case CONF_UNQUEUED:
		cli_msg(17,
			"Reconfiguration already in progress, removing queued config");
		break;
	case CONF_CONFIRM:
		cli_msg(18, "Reconfiguration confirmed");
		break;
	case CONF_SHUTDOWN:
		cli_msg(6, "Reconfiguration ignored, shutting down");
		break;
	case CONF_NOTHING:
		cli_msg(19, "Nothing to do");
		break;
	default:
		break;
	}
...
struct cli *cmd_reconfig_stored_cli;

void cmd_reconfig_undo_notify(void)
{
	if (cmd_reconfig_stored_cli) {
		struct cli *c = cmd_reconfig_stored_cli;
		cli_printf(c, CLI_ASYNC_CODE,
...
static struct birdsock *cli_sk;
static char *path_control_socket = PATH_CONTROL_SOCKET;

static void cli_write(struct cli *c)
{
	struct birdsock *s = c->priv;
...
void cli_write_trigger(struct cli *c)
{
	struct birdsock *s = c->priv;
...
static void cli_tx(struct birdsock *s)
{
	cli_write(s->data);
}

int cli_get_command(struct cli *c)
{
	struct birdsock *s = c->priv;
...
static int cli_rx(struct birdsock *s, int size UNUSED)
{
	cli_kick(s->data);
	return 0;
}

static void cli_err(struct birdsock *s, int err)
{
	if (config->cli_debug) {
		if (err)
...
static int cli_connect(struct birdsock *s, int size UNUSED)
{
	struct cli *c;
	...
	s->err_hook = cli_err;
	s->data = c = cli_new(s);
	s->pool = c->pool;	/* We need to have all the socket buffers allocated in the struct cli struct pool */
	s->fast_rx = 1;
	c->rx_pos = c->rx_buf;
	c->rx_aux = NULL;
	rmove(s, c->pool);
...
static inline void open_pid_file(void)
{
	if (!pid_file)
		return;
...
static inline void write_pid_file(void)
{
	int pl, rv;
	char ps[24];
	...
	if (pl < 0)
		bug("PID struct buffer too small");
...
static inline void unlink_pid_file(void)
{
	if (pid_file)
		unlink(pid_file);
}
...
static void handle_sighup(int sig UNUSED)
{
	DBG("Caught SIGHUP...\n");
	async_config_flag = 1;
}

static void handle_sigusr(int sig UNUSED)
{
	DBG("Caught SIGUSR...\n");
	async_dump_flag = 1;
}

static void handle_sigterm(int sig UNUSED)
{
	DBG("Caught SIGTERM...\n");
...
{
	fprintf(stderr,
		"Usage: %s [-c <config-file>] [-d] [-D <debug-file>] [-p] [-s <control-socket>] [-P <pid-file>] [-u <user>] [-g <group>] [-f] [-l] [-R]\n",
		bird_name);
	exit(1);
}

static inline char *get_bird_name(char *s, char *def)
{
	char *t;
	if (!s)
		return def;
...
static inline uid_t get_uid(const char *s)
{
	struct passwd *pw;
	char *endptr;
	long int rv;
...
static inline gid_t get_gid(const char *s)
{
	struct group *gr;
	char *endptr;
	long int rv;
