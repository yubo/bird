...
int ip6_classify(struct ip6_addr * a)
{
	...
	if ((x & 0xff000000) == 0xff000000) {	/* ff00::/8  Multicast Address */
		...
		switch (scope) {
		case 1:
			return IADDR_MULTICAST | SCOPE_HOST;
		case 2:
			return IADDR_MULTICAST | SCOPE_LINK;
		case 5:
			return IADDR_MULTICAST | SCOPE_SITE;
		case 8:
			return IADDR_MULTICAST | SCOPE_ORGANIZATION;
		case 14:
			return IADDR_MULTICAST | SCOPE_UNIVERSE;
		default:
			return IADDR_MULTICAST | SCOPE_UNDEFINED;
		}
	}
...
{
	u32 x = _I(a);
	return b + bsprintf(b, "%d.%d.%d.%d", (x >> 24) & 0xff,
...
char *ip6_ntop(struct ip6_addr a, char *b)
{
	...
	if (!bestpos
	    && ((bestlen == 5 && a.addr[2] == 0xffff) || (bestlen == 6))) {
		u32 x = a.addr[3];
...
int ip6_pton(const char *a, struct ip6_addr * o)
{
	...
	/* Convert the address to struct ip6_addr format */
	for (i = 0; i < 4; i++)
 ...
 */
char *ip_scope_text(uint scope)
{
	static char *scope_table[] =
 ...
 */
int ipa_equal(ip_addr x, ip_addr y)
{
DUMMY}
 ...
 */
ip_addr ipa_and(ip_addr x, ip_addr y)
{
DUMMY}
 ...
 */
ip_addr ipa_or(ip_addr x, ip_addr y)
{
DUMMY}
 ...
 */
ip_addr ipa_xor(ip_addr x, ip_addr y)
{
DUMMY}
 ...
 */
struct ip4_addr ip4_class_mask(struct ip4_addr x)
{
DUMMY}
 ...
 */
int ipa_compare(ip_addr x, ip_addr y)
{
DUMMY}
 ...
 */
ip_addr ipa_build6(u32 a1, u32 a2, u32 a3, u32 a4)
{
DUMMY}
 ...
 */
char *ip_ntox(ip_addr a, char *buf)
{
DUMMY}
