 ...
 *
 * We use FIB struct node flags in the routing table to keep track of route
...
static union list krt_proto_list;
...
static inline void
krt_trace_in(struct krt_proto *p, struct rte *e, char *msg)
{
...
static inline int
krt_same_key(struct rte *a, struct rte *b)
{
  return a->u.krt.metric == b->u.krt.metric;
}

static inline int
krt_uptodate(struct rte *a, struct rte *b)
{
  if (a->attrs != b->attrs)
    return 0;
...
static int
krt_same_dest(struct rte *k, struct rte *e)
{
...
static void
krt_scan(struct timer *t UNUSED)
{
  struct krt_proto *p;

  kif_force_scan();

  /* We need some struct node to decide whether to print the debug messages or not */
...
static int
krt_import_control(struct proto *P, struct rte **new, struct ea_list **attrs, struct linpool *pool)
{
  struct krt_proto *p = (struct krt_proto *) P;
  struct rte *e = *new;
...
static int
krt_rte_same(struct rte *a, struct rte *b)
{
...
static int
krt_get_attr(struct eattr *a, byte *buf, int buflen)
{
  switch (a->id)
  {
  case EA_KRT_SOURCE:
