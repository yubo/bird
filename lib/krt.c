 ...
 *
 * We use FIB struct list_head flags in the routing table to keep track of route
...
struct pool *krt_pool;
static struct linpool *krt_filter_lp;
static struct list_head krt_proto_list;

void krt_io_init(void)
{
	krt_pool = rp_new(&root_pool, "Kernel Syncer");
	krt_filter_lp = lp_new(krt_pool, 4080);
	INIT_LIST_HEAD(&krt_proto_list);
	krt_sys_io_init();
}
...
static struct timer *kif_scan_timer;
static bird_clock_t kif_last_shot;
uint32_t kif_flag;

static void kif_scan(struct timer *t)
{
...
static void kif_force_scan(void)
{
	if (kif_proto && kif_last_shot + 2 < now) {
		kif_scan(kif_scan_timer);
		tm_start(kif_scan_timer,
			 ((struct kif_config *)kif_proto->p.cf)->scan_time);
	}
}
...
static inline int prefer_addr(struct ifa *a, struct ifa *b)
{
...
static inline struct ifa *find_preferred_ifa(struct iface *i, ip_addr prefix,
		ip_addr mask)
{
	struct ifa *a, *b = NULL;
...
struct ifa *kif_choose_primary(struct iface *i)
{
	...
	list_for_each_entry(it, &cf->primary, n) {
		if (!it->pattern || patmatch(it->pattern, i->name))
			if (a = find_preferred_ifa(i, it->prefix,
					       ipa_mkmask(it->pxlen)))
				return a;
	}
...
static struct proto *kif_init(struct proto_config *c)
{
...
static int kif_start(struct proto *P)
{
	struct kif_proto *p = (struct kif_proto *)P;

	kif_proto = p;

	if (kif_flag & KIF_F_SCAN_DISABLE)
		goto out;
...
static int kif_shutdown(struct proto *P)
{
	struct kif_proto *p = (struct kif_proto *)P;
...
static int kif_reconfigure(struct proto *p, struct proto_config *new)
{
	...
	if (!list_empty(&o->primary) || !list_empty(&n->primary)) {
...
static void kif_preconfig(struct protocol *P UNUSED, struct config *c)
{
	kif_cf = NULL;
	kif_sys_preconfig(c);
}
...
static void kif_copy_config(struct proto_config *dest, struct proto_config *src)
{
	...
	/* Copy primary addr struct list_head */
	cfg_copy_list(&d->primary, &s->primary,
...
static inline void krt_trace_in(struct krt_proto *p, struct rte *e, char *msg)
{
	if (p->p.debug & D_PACKETS)
		log(L_TRACE "%s: %I/%d: %s", p->p.name, e->net->n.prefix,
		    e->net->n.pxlen, msg);
}

static inline void
krt_trace_in_rl(struct tbf *f, struct krt_proto *p, struct rte *e, char *msg)
{
	if (p->p.debug & D_PACKETS)
		log_rl(f, L_TRACE "%s: %I/%d: %s", p->p.name, e->net->n.prefix,
		       e->net->n.pxlen, msg);
}
...
static inline int krt_same_key(struct rte *a, struct rte *b)
{
	return a->u.krt.metric == b->u.krt.metric;
}

static inline int krt_uptodate(struct rte *a, struct rte *b)
{
	if (a->attrs != b->attrs)
		return 0;
...
static void krt_learn_announce_update(struct krt_proto *p, struct rte *e)
{
...
static void krt_learn_announce_delete(struct krt_proto *p, struct network *n)
{
...
static void krt_learn_scan(struct krt_proto *p, struct rte *e)
{
...
static void krt_learn_prune(struct krt_proto *p)
{
...
static void krt_learn_async(struct krt_proto *p, struct rte *e, int new)
{
	struct network *n0 = e->net;
	struct network *n = net_get(&p->krt_table, n0->n.prefix, n0->n.pxlen);
...
static void krt_learn_init(struct krt_proto *p)
{
	if (KRT_CF->learn)
		rt_setup(p->p.pool, &p->krt_table, "Inherited", NULL);
}

static void krt_dump(struct proto *P)
{
...
static void krt_dump_attrs(struct rte *e)
{
...
static void krt_flush_routes(struct krt_proto *p)
{
...
static struct rte *krt_export_net(struct krt_proto *p, struct network *net,
				  struct rte **rt_free, struct ea_list **tmpa)
{
	...
	*tmpa =
	    src->make_tmp_attrs ? src->make_tmp_attrs(rt, krt_filter_lp) : NULL;
	...
	if (f_run(filter, &rt, tmpa, krt_filter_lp, FF_FORCE_TMPATTR) >
	    F_ACCEPT)
		goto reject;
...
static int krt_same_dest(struct rte *k, struct rte *e)
{
	struct rta *ka = k->attrs, *ea = e->attrs;
 ...
 *  We expect that the route is a temporary struct rte and its attributes are uncached.
 */
...
sentenced:
	krt_trace_in(p, e, ((char *[]) {
			    "?", "seen", "will be updated", "will be removed",
			    "ignored"})[verdict]);
	...
	FIB_WALK(&t->fib, f) {
		...
		if (verdict == KRF_CREATE || verdict == KRF_UPDATE) {
			...
			if (!new)
				verdict =
				    (verdict ==
				     KRF_CREATE) ? KRF_IGNORE : KRF_DELETE;
			else
...
static void krt_scan(struct timer *t)
{
...
{
	p->scan_timer =
	    tm_new_set(p->p.pool, krt_scan, p, 0, KRT_CF->scan_time);
...
static void krt_scan_timer_kick(struct krt_proto *p)
{
...
static struct ea_list *krt_make_tmp_attrs(struct rte *rt, struct linpool *pool)
{
	struct ea_list *l =
	    lp_alloc(pool, sizeof(struct ea_list) + 2 * sizeof(struct eattr));
...
static void krt_store_tmp_attrs(struct rte *rt, struct ea_list *attrs)
{
...
static int
krt_import_control(struct proto *P, struct rte **new, struct ea_list **attrs,
		   struct linpool *pool)
{
	struct krt_proto *p = (struct krt_proto *)P;
	struct rte *e = *new;
...
static int krt_reload_routes(struct proto *P)
{
	struct krt_proto *p = (struct krt_proto *)P;
...
static void krt_feed_end(struct proto *P)
{
	struct krt_proto *p = (struct krt_proto *)P;
...
static int krt_rte_same(struct rte *a, struct rte *b)
{
...
static struct proto *krt_init(struct proto_config *C)
{
...
static int krt_start(struct proto *P)
{
	struct krt_proto *p = (struct krt_proto *)P;
...
static int krt_shutdown(struct proto *P)
{
	struct krt_proto *p = (struct krt_proto *)P;
...
static int krt_reconfigure(struct proto *p, struct proto_config *new)
{
...
static void krt_preconfig(struct protocol *P UNUSED, struct config *c)
{
	krt_cf = NULL;
	krt_sys_preconfig(c);
}

static void krt_postconfig(struct proto_config *C)
{
	struct krt_config *c = (struct krt_config *)C;
	...
	if (C->table->krt_attached)
		cf_error("Kernel syncer (%s) already attached to table %s",
			 C->table->krt_attached->name, C->table->name);
...
static void krt_copy_config(struct proto_config *dest, struct proto_config *src)
{
...
static int krt_get_attr(struct eattr *a, byte * buf, int buflen)
{
	switch (a->id) {
	case EA_KRT_SOURCE:
		bsprintf(buf, "source");
