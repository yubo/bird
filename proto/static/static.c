 ...
 * pointing to a neighboring router use a sticky struct list_head in the struct neighbor cache
 ...
 * static_route struct list_head (of dest RTD_MULTIPATH), which specifies prefix
 * and is used in most circumstances, and a struct list_head of dummy static_route
...
static inline struct rtable *p_igp_table(struct proto *p)
{
...
static_install(struct proto *p, struct static_route *r, struct iface *ifa)
{
	...
	if (r->dest == RTDX_RECURSIVE)
		rta_set_recursive_next_hop(p->table, &a, p_igp_table(p),
					   &r->via, &r->via);
...
static void static_remove(struct proto *p, struct static_route *r)
{
	struct network *n;
...
static void static_bfd_notify(struct bfd_request *req);

static void static_update_bfd(struct proto *p, struct static_route *r)
{
	...
	if (bfd_up && !r->bfd_req) {
		...
		r->bfd_req =
		    bfd_request_session(p->pool, r->via, nb->ifa->ip, nb->iface,
					static_bfd_notify, r);
	}
...
static int static_decide(struct static_config *cf, struct static_route *r)
{
...
static_add(struct proto *p, struct static_config *cf, struct static_route *r)
{
	...
	case RTD_ROUTER:
		{
			struct neighbor *n =
			    neigh_find2(p, &r->via, r->via_if, NEF_STICKY);
			if (n) {
				r->chain = n->data;
				n->data = r;
				r->neigh = n;
				...
				log(L_ERR
				    "Static route destination %I is invalid. Ignoring.",
				    r->via);
				static_remove(p, r);
			}
			break;
	...
	case RTD_MULTIPATH:
		{
			...
			for (r2 = r->mp_next; r2; r2 = r2->mp_next) {
				struct neighbor *n =
				    neigh_find2(p, &r2->via, r2->via_if,
						NEF_STICKY);
				if (n) {
					r2->chain = n->data;
					n->data = r2;
...
static void static_rte_cleanup(struct proto *p, struct static_route *r)
{
	struct static_route *r2;
...
static int static_start(struct proto *p)
{
	struct static_config *cf = (void *)p->cf;
...
static int static_shutdown(struct proto *p)
{
	struct static_config *cf = (void *)p->cf;
	struct static_route *r;
...
static void static_cleanup(struct proto *p)
{
	struct static_config *cf = (void *)p->cf;
...
static void static_update_rte(struct proto *p, struct static_route *r)
{
	switch (r->dest) {
	case RTD_ROUTER:
...
static void static_neigh_notify(struct neighbor *n)
{
...
static void static_dump_rt(struct static_route *r)
{
...
static void static_dump(struct proto *p)
{
...
static void static_if_notify(struct proto *p, unsigned flags, struct iface *i)
{
	struct static_route *r;
	struct static_config *c = (void *)p->cf;
...
int static_rte_mergable(struct rte *pri, struct rte *sec)
{
	return 1;
}

void static_init_config(struct static_config *c)
{
	INIT_LIST_HEAD(&c->iface_routes);
	INIT_LIST_HEAD(&c->other_routes);
}

static struct proto *static_init(struct proto_config *c)
{
...
static inline struct rtable *cf_igp_table(struct static_config *cf)
{
	return cf->igp_table ? cf->igp_table->table : NULL;
}

static int static_reconfigure(struct proto *p, struct proto_config *new)
{
...
static void static_copy_routes(struct list_head *dlst, struct list_head *slst)
{
...
static void static_show_rt(struct static_route *r)
{
