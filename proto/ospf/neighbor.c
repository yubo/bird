...
const char *ospf_inm_names[] = {
	"HelloReceived", "Start", "2-WayReceived", "NegotiationDone",
	    "ExchangeDone",
	"BadLSReq", "LoadingDone", "AdjOK?", "SeqNumberMismatch",
	    "1-WayReceived",
	"KillNbr", "InactivityTimer", "LLDown"
};

static int can_do_adj(struct ospf_neighbor *n);
static void inactivity_timer_hook(struct timer * timer);
static void dbdes_timer_hook(struct timer * t);
static void lsrq_timer_hook(struct timer * t);
static void lsrt_timer_hook(struct timer * t);
static void ackd_timer_hook(struct timer * t);
 ...
 */
static void ospf_neigh_chstate(struct ospf_neighbor *n, u8 state)
{
	struct ospf_iface *ifa = n->ifa;
	struct ospf_proto *p = ifa->oa->po;
	...
	OSPF_TRACE(D_EVENTS, "Neighbor %R on %s changed state from %s to %s",
		   n->rid, ifa->ifname, ospf_ns_names[old_state],
		   ospf_ns_names[state]);
	...
	if (ifa->fadj != old_fadj) {
		/* RFC 2328 12.4 Event 4 - struct neighbor enters/leaves Full state */
	...
	/* Generate NeighborChange struct event if needed, see RFC 2328 9.2 */
...
/**
 * ospf_neigh_sm - ospf struct neighbor state machine
 * @n: neighor
 * @event: actual event
 *
 * This part implements the struct neighbor state machine as described in 10.3 of
...
void ospf_neigh_sm(struct ospf_neighbor *n, int event)
{
	...
	DBG("Neighbor state machine for %R on %s, struct event %s\n",
	...
	case INM_HELLOREC:
		...
		/* Restart inactivity struct timer */
...
static int can_do_adj(struct ospf_neighbor *n)
{
	...
	case OSPF_IT_NBMA:
		...
		case OSPF_IS_DOWN:
		case OSPF_IS_LOOP:
			bug("%s: Iface %s in down state?", p->p.name,
			    ifa->ifname);
			break;
		...
		case OSPF_IS_DR:
			...
			break;
		default:
			bug("%s: Iface %s in unknown state?", p->p.name,
			    ifa->ifname);
			break;
		}
		break;
...
static void inactivity_timer_hook(struct timer * timer)
{
	...
	OSPF_TRACE(D_EVENTS, "Inactivity struct timer expired for nbr %R on %s",
		   n->rid, n->ifa->ifname);
	ospf_neigh_sm(n, INM_INACTTIM);
}

static void ospf_neigh_bfd_hook(struct bfd_request *req)
{
...
static void dbdes_timer_hook(struct timer * t)
{
	...
	// OSPF_TRACE(D_EVENTS, "DBDES struct timer expired for nbr %R on %s", n->rid, n->ifa->ifname);
...
void ospf_sh_neigh_info(struct ospf_neighbor *n)
{
	...
	cli_msg(-1013, "%-1R\t%3u\t%s/%s\t%-5s\t%-10s %-1I", n->rid,
		n->priority, ospf_ns_names[n->state], pos, etime, ifa->ifname,
		n->ip);
}
