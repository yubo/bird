...
void ospf_dump_lsahdr(struct ospf_proto *p, struct ospf_lsa_header *lsa_n)
{
	...
	log(L_TRACE
	    "%s:     LSA      Type: %04x, Id: %R, Rt: %R, Seq: %08x, Age: %u, Sum: %04x",
	    p->p.name, lsa_etype, lsa.id, lsa.rt, lsa.sn, lsa.age,
	    lsa.checksum);
}
...
static inline uint ospf_lsupd_hdrlen(struct ospf_proto *p)
{
	return ospf_pkt_hdrlen(p) + 4;	/* + u32 lsa count field */
}

static inline u32 ospf_lsupd_get_lsa_count(struct ospf_packet *pkt, uint hdrlen)
{
...
static void ospf_dump_lsupd(struct ospf_proto *p, struct ospf_packet *pkt)
{
...
{
	if (req == n->lsrqi)
		n->lsrqi = HLIST_NEXT(req);

	h_rem_node((struct hlist *)req);
...
ospf_lsa_lsrt_up(struct top_hash_entry *en, struct ospf_neighbor *n)
{
	...
	//if (!SNODE_VALID(ret)) {
	if (list_empty(&ret->n.n)) {
		en->ret_count++;
		//s_add_tail(&n->lsrtl, SNODE ret);
		h_add_tail(&n->lsrtl, HLIST ret);
	}
		    ...
		    struct top_hash_entry *ret)
{
	...
	h_rem_node(HLIST ret);

	ospf_hash_delete(n->lsrth, ret);

	if (list_empty(&n->lsrtl.n))
...
static int ospf_flood_lsupd(struct ospf_proto *p,
			    struct top_hash_entry **lsa_list, uint lsa_count,
			    uint lsa_min_count, struct ospf_iface *ifa);

static void
ospf_enqueue_lsa(struct ospf_proto *p, struct top_hash_entry *en,
		 struct ospf_iface *ifa)
{
	if (ifa->flood_queue_used == ifa->flood_queue_size) {
		/* If we already have full queue, we send some packets */
		uint sent =
		    ospf_flood_lsupd(p, ifa->flood_queue, ifa->flood_queue_used,
				     ifa->flood_queue_used / 2, ifa);
		int i;
		...
		memmove(ifa->flood_queue, ifa->flood_queue + sent,
			ifa->flood_queue_used * sizeof(void *));
		bzero(ifa->flood_queue + ifa->flood_queue_used,
		      sent * sizeof(void *));
	}
 ...
 * @en: LSA entry
 * @from: struct neighbor than sent this LSA (or NULL if LSA is local)
 *
 * return value - was the LSA flooded back?
 */
int
ospf_flood_lsa(struct ospf_proto *p, struct top_hash_entry *en,
	       struct ospf_neighbor *from)
{
	struct ospf_iface *ifa;
		...
		list_for_each_entry(n, &ifa->neigh_list, n) {
			...
			if (n->state < NEIGHBOR_FULL) {
				...
				if (req != NULL) {
					...
					/* If same or newer, remove LSA from the link
					 * state request struct list_head */
					if (cmp > CMP_OLDER)
						ospf_lsa_lsrq_down(req, n, from);

					/* If older or same, skip processing of this struct neighbor */
					if (cmp < CMP_NEWER)
						continue;
				}
			}
		   ...
		   struct top_hash_entry **lsa_list, uint lsa_count)
{
	...
	for (i = 0; i < lsa_count; i++) {
		...
		if ((pos + len) > maxsize) {
			...
			/* LSA is larger than MTU, check struct buffer size */
			if (ospf_iface_assure_bufsize(ifa, pos + len) < 0) {
				/* Cannot fit in a tx buffer, skip that */
				log(L_ERR
				    "%s: LSA too large to send on %s (Type: %04x, Id: %R, Rt: %R)",
				    p->p.name, ifa->ifname, en->lsa_type,
				    en->lsa.id, en->lsa.rt);
				break;
			}

			/* TX struct buffer could be reallocated */
			pkt = ospf_tx_buffer(ifa);
		}

		struct ospf_lsa_header *buf = ((void *)pkt) + pos;
		lsa_hton_hdr(&en->lsa, buf);
		lsa_hton_body(en->lsa_body,
			      ((void *)buf) + sizeof(struct ospf_lsa_header),
			      len - sizeof(struct ospf_lsa_header));
		buf->age =
		    htons(MIN(en->lsa.age + ifa->inftransdelay, LSA_MAXAGE));
...
static int
ospf_flood_lsupd(struct ospf_proto *p, struct top_hash_entry **lsa_list,
		 uint lsa_count, uint lsa_min_count, struct ospf_iface *ifa)
{
	uint i, c;
...
int
ospf_send_lsupd(struct ospf_proto *p, struct top_hash_entry **lsa_list,
		uint lsa_count, struct ospf_neighbor *n)
{
	...
	for (i = 0; i < lsa_count; i += c) {
		...
		OSPF_PACKET(ospf_dump_lsupd, ospf_tx_buffer(ifa),
			    "LSUPD packet sent to nbr %R on %s", n->rid,
			    ifa->ifname);
...
void ospf_rxmt_lsupd(struct ospf_proto *p, struct ospf_neighbor *n)
{
	...
	list_for_each_safe(_p, _n, &n->lsrtl.n) {
		ret = (void *)_p;
		if (i == max)
			break;
		   ...
		   struct ospf_neighbor *n)
{
	...
	if (plen < ospf_lsupd_hdrlen(p)) {
		LOG_PKT("Bad LSUPD packet from nbr %R on %s - %s (%u)", n->rid,
			ifa->ifname, "too short", plen);
		return;
	}

	OSPF_PACKET(ospf_dump_lsupd, pkt,
		    "LSUPD packet received from nbr %R on %s", n->rid,
		    ifa->ifname);

	if (n->state < NEIGHBOR_EXCHANGE) {
		OSPF_TRACE(D_PACKETS,
			   "LSUPD packet ignored - lesser state than Exchange");
		return;
	}
	...
	for (i = 0; i < lsa_count; i++) {
		...
		if ((lsa_n->checksum == 0)
		    || !lsa_verify_checksum(lsa_n, lsa_len))
		...
		if (!oa_is_ext(ifa->oa)
		    && (LSA_SCOPE(lsa_type) == LSA_SCOPE_AS))
		...
		en = ospf_hash_find(p->gr, lsa_domain, lsa.id, lsa.rt,
				    lsa_type);
		...
		if (!en || (lsa_comp(&lsa, &en->lsa) == CMP_NEWER)) {
			...
			if (lsa_validate(&lsa, lsa_type, ospf_is_v2(p), body) ==
			    0) {
				mb_free(body);
				SKIP("invalid body");
			}
			...
			if ((lsa.rt == p->router_id) ||
			    (ospf_is_v2(p) && (lsa_type == LSA_T_NET)
			     && ospf_addr_is_local(p, ifa->oa,
						   ipa_from_u32(lsa.id)))) {
				OSPF_TRACE(D_EVENTS,
					   "Received unexpected self-originated LSA");
				ospf_advance_lsa(p, en, &lsa, lsa_type,
						 lsa_domain, body);
				continue;
			}
			...
			en = ospf_install_lsa(p, &lsa, lsa_type, lsa_domain,
					      body);
		...
		/* 13. (8) - received LSA is older */
		{
			...
			/* Send newer local copy back to struct neighbor */
...
skip:
		LOG_LSA1("Bad LSA (Type: %04x, Id: %R, Rt: %R) in LSUPD",
			 lsa_type, lsa.id, lsa.rt);
		LOG_LSA2("  received from nbr %R on %s - %s", n->rid,
			 ifa->ifname, err_dsc);
	}
