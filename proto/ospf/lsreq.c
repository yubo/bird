...
static void ospf_dump_lsreq(struct ospf_proto *p, struct ospf_packet *pkt)
{
	struct ospf_lsreq_header *lsrs;
	uint i, lsr_count;
	...
	for (i = 0; i < lsr_count; i++)
		log(L_TRACE "%s:     LSR      Type: %04x, Id: %R, Rt: %R",
		    p->p.name, ntohl(lsrs[i].type), ntohl(lsrs[i].id),
		    ntohl(lsrs[i].rt));
}
...
void ospf_send_lsreq(struct ospf_proto *p, struct ospf_neighbor *n)
{
	...
	OSPF_PACKET(ospf_dump_lsreq, pkt, "LSREQ packet sent to nbr %R on %s",
		    n->rid, ifa->ifname);
		   ...
		   struct ospf_neighbor *n)
{
	...
	OSPF_PACKET(ospf_dump_lsreq, pkt,
		    "LSREQ packet received from nbr %R on %s", n->rid,
		    ifa->ifname);

	if (n->state < NEIGHBOR_EXCHANGE) {
		OSPF_TRACE(D_PACKETS,
			   "LSREQ packet ignored - lesser state than Exchange");
		return;
	}
	...
	for (i = 0; i < lsr_count; i++) {
		...
		if (!en) {
			LOG_LSA1
			    ("Bad LSR (Type: %04x, Id: %R, Rt: %R) in LSREQ",
			     type, id, rt);
			LOG_LSA2
			    ("  received from nbr %R on %s - LSA is missing",
			     n->rid, ifa->ifname);
