 ...
 * data structures. Source |neighbor.c| includes the struct neighbor state machine and
 ...
 * these from 'parent' &ospf_iface) and it is present in iface struct list_head even when
...
static int ospf_import_control(struct proto *P, struct rte ** new,
		struct ea_list ** attrs, struct linpool *pool);
static struct ea_list *ospf_make_tmp_attrs(struct rte *rt,
		struct linpool *pool);
...
static void ospf_disp(struct timer * timer);

static void ospf_area_initfib(struct fib_node *fn)
{
...
static void add_area_nets(struct ospf_area *oa, struct ospf_area_config *ac)
{
	struct ospf_proto *p = oa->po;
	struct area_net_config *anc;
	struct area_net *an;

	fib_init(&oa->net_fib, p->p.pool, sizeof(struct area_net), 0,
		 ospf_area_initfib);
	fib_init(&oa->enet_fib, p->p.pool, sizeof(struct area_net), 0,
		 ospf_area_initfib);

	list_for_each_entry(anc, &ac->net_list, n) {
		an = (struct area_net *)fib_get(&oa->net_fib, &anc->px.addr,
						anc->px.len);
		an->hidden = anc->hidden;
	}

	list_for_each_entry(anc, &ac->enet_list, n) {
		an = (struct area_net *)fib_get(&oa->enet_fib, &anc->px.addr,
						anc->px.len);
		an->hidden = anc->hidden;
		an->tag = anc->tag;
	}
...
static void ospf_area_add(struct ospf_proto *p, struct ospf_area_config *ac)
{
	struct ospf_area *oa;
	...
	list_add_tail(&oa->n, &p->area_list);
	p->areano++;

	oa->ac = ac;
	oa->areaid = ac->areaid;
	oa->rt = NULL;
	INIT_LIST_HEAD(&oa->cand);
	oa->po = p;
	fib_init(&oa->rtr, p->p.pool, sizeof(struct ort), 0, ospf_rt_initort);
	add_area_nets(oa, ac);
...
static struct ospf_iface *ospf_find_vlink(struct ospf_proto *p, u32 voa,
					  u32 vid)
{
	struct ospf_iface *ifa;
	list_for_each_entry(ifa, &p->iface_list, n)
	    if ((ifa->type == OSPF_IT_VLINK) && (ifa->voa->areaid == voa)
		&& (ifa->vid == vid))
		return ifa;
	return NULL;
}
...
log(L_TRACE "---------- ospf_start");
	...
	INIT_LIST_HEAD(&(p->iface_list));
	INIT_LIST_HEAD(&(p->area_list));
	...
	p->log_pkt_tbf = (struct tbf) {
	.rate = 1,.burst = 5};
	p->log_lsa_tbf = (struct tbf) {
	.rate = 4,.burst = 20};

	list_for_each_entry(ac, &c->area_list, n)
	    ospf_area_add(p, ac);
	...
	list_for_each_entry(ip, &c->vlink_list, n)
	    ospf_iface_new_vlink(p, (struct ospf_iface_patt *)ip);
...
static void ospf_dump(struct proto *P)
{
	...
	list_for_each_entry(ifa, &p->iface_list, n) {
		...
		list_for_each_entry(n, &ifa->neigh_list, n) {
			OSPF_TRACE(D_EVENTS, "  struct neighbor %R in state %u",
				   n->rid, n->state);
		}
	}
...
static int ospf_rte_better(struct rte *new, struct rte *old)
{
	...
	if (new->attrs->source < old->attrs->source)
		return 1;
	if (new->attrs->source > old->attrs->source)
		return 0;
...
static struct ea_list *ospf_build_attrs(struct ea_list * next, struct linpool *pool, u32 m1,
				 u32 m2, u32 tag, u32 rid)
{
	struct ea_list *l =
	    lp_alloc(pool, sizeof(struct ea_list) + 4 * sizeof(struct eattr));
 ...
 * @timer: struct timer usually called every @ospf_proto->tick second, @timer->data
 * point to @ospf_proto
 */
static void ospf_disp(struct timer * timer)
{
 ...
 * @new: the new route
 * @attrs: struct list_head of attributes
 * @pool: struct pool for allocation of attributes
 *
 * Its quite simple. It does not accept our own routes and leaves the decision on
 * import to the filters.
 */
static int
ospf_import_control(struct proto *P, struct rte ** new, struct ea_list ** attrs,
		    struct linpool *pool)
{
	struct ospf_proto *p = (struct ospf_proto *)P;
	...
	struct ea_list *ea = e->attrs->eattrs;
...
{
	return ospf_build_attrs(NULL, pool, rt->u.ospf.metric1,
				rt->u.ospf.metric2, rt->u.ospf.tag,
				rt->u.ospf.router_id);
}
 ...
 * them hello packet with empty struct neighbor list. They should start
 * their struct neighbor state machine with struct event %NEIGHBOR_1WAY.
 */
static int ospf_shutdown(struct proto *P)
{
	struct ospf_proto *p = (struct ospf_proto *)P;
	struct ospf_iface *ifa;
	...
	/* Cleanup locked struct rta entries */
	FIB_WALK(&p->rtf, nftmp) {
		rta_free(((struct ort *) nftmp)->old_rta);
	}
	FIB_WALK_END;
...
static void ospf_get_status(struct proto *P, byte * buf)
{
	struct ospf_proto *p = (struct ospf_proto *)P;
	...
	if (p->p.proto_state == PS_DOWN)
		...
		list_for_each_entry(ifa, &p->iface_list, n)
		    list_for_each_entry(n, &ifa->neigh_list, n) if (n->state == NEIGHBOR_FULL)
			adj = 1;
...
static void ospf_get_route_info(struct rte * rte, byte * buf, struct ea_list * attrs UNUSED)
{
	char *type = "<bug>";
	...
	buf += bsprintf(buf, ")");
	if ((rte->attrs->source == RTS_OSPF_EXT1
	     || rte->attrs->source == RTS_OSPF_EXT2) && rte->u.ospf.tag) {
...
static int ospf_get_attr(struct eattr * a, byte * buf, int buflen UNUSED)
{
	switch (a->id) {
 ...
 * password change, addition or deletion of some struct neighbor on
 * nonbroadcast network, cost of interface, etc.
 */
static int ospf_reconfigure(struct proto *P, struct proto_config *c)
{
	struct ospf_proto *p = (struct ospf_proto *)P;
...
log(L_TRACE "---------- ospf_reconfigure");
	...
	list_for_each_entry(nac, &new->area_list, n) {
	...
	list_for_each_entry(ip, &new->vlink_list, n) {
		oip = (struct ospf_iface_patt *)ip;
		ifa = ospf_find_vlink(p, oip->voa, oip->vid);
		if (ifa)
			ospf_iface_reconfigure(ifa, oip);
		else
			ospf_iface_new_vlink(p, oip);
	}
	...
	list_for_each_entry_safe(oa, oax, &p->area_list, n)
	    if (oa->marked)
		ospf_area_remove(oa);
...
void ospf_sh_neigh(struct proto *P, char *iff)
{
	...
	cli_msg(-1013, "%-12s\t%3s\t%-15s\t%-5s\t%-10s %-12s", "Router ID",
		"Pri", "     State", "DTime", "Interface", "Router IP");
	list_for_each_entry(ifa, &p->iface_list, n)
...
void ospf_sh(struct proto *P)
{
	...
	cli_msg(-1014, "RFC1583 compatibility: %s",
		(p->rfc1583 ? "enabled" : "disabled"));
	...
	list_for_each_entry(oa, &p->area_list, n) {
		...
		if (oa_is_nssa(oa))
			cli_msg(-1014, "\t\tNSSA translation:\t%s%s",
				oa->translate ? "Yes" : "No",
				oa->translate ==
				TRANS_WAIT ? " (run down)" : "");
		...
		FIB_WALK(&oa->net_fib, nftmp) {
			...
			cli_msg(-1014, "\t\t\t%1I/%u\t%s\t%s", anet->fn.prefix,
				anet->fn.pxlen,
 ...
 * router struct list_head together). Then, according to LSA type, ID and age.
 *
...
static struct ospf_lsa_header *fake_lsa_from_prefix_lsa(struct ospf_lsa_header
							*dst,
...
static int lsa_compare_for_state(const void *p1, const void *p2)
{
...
static int ext_compare_for_state(const void *p1, const void *p2)
{
	struct top_hash_entry *he1 = *(struct top_hash_entry **)p1;
	struct top_hash_entry *he2 = *(struct top_hash_entry **)p2;
	struct ospf_lsa_header *lsa1 = &(he1->lsa);
...
static inline void show_lsa_distance(struct top_hash_entry *he)
{
	if (he->color == INSPF)
...
show_lsa_router(struct ospf_proto *p, struct top_hash_entry *he, int verbose)
{
	...
	while (lsa_walk_rt(&rtl))
		if (rtl.type == LSART_VLNK)
			cli_msg(-1016, "\t\tvlink %R metric %u", rtl.id,
				rtl.metric);

	lsa_walk_rt_init(p, he, &rtl);
	while (lsa_walk_rt(&rtl))
		if (rtl.type == LSART_PTP)
			cli_msg(-1016, "\t\trouter %R metric %u", rtl.id,
				rtl.metric);
		...
		if (rtl.type == LSART_NET) {
			if (ospf_is_v2(p)) {
				...
				struct top_hash_entry *net_he =
				    ospf_hash_find_net2(p->gr, he->domain,
							rtl.id);

				if (net_he && (net_he->lsa.age < LSA_MAXAGE)) {
					struct ospf_lsa_header *net_lsa =
					    &(net_he->lsa);
					struct ospf_lsa_net *net_ln =
					    net_he->lsa_body;

					cli_msg(-1016,
						"\t\tnetwork %I/%d metric %u",
						ipa_from_u32(net_lsa->
							     id & net_ln->optx),
						u32_masklen(net_ln->optx),
						rtl.metric);
				} else
					cli_msg(-1016,
						"\t\tnetwork [%R] metric %u",
				...
				cli_msg(-1016, "\t\tnetwork [%R-%u] metric %u",
					rtl.id, rtl.nif, rtl.metric);
		}
		...
		while (lsa_walk_rt(&rtl))
			if (rtl.type == LSART_STUB)
				cli_msg(-1016, "\t\tstubnet %I/%d metric %u",
					ipa_from_u32(rtl.id),
					u32_masklen(rtl.data), rtl.metric);
	}
}

static inline void show_lsa_network(struct top_hash_entry *he, int ospf2)
{
	...
	if (ospf2) {
		cli_msg(-1016, "");
		cli_msg(-1016, "\tnetwork %I/%d",
			ipa_from_u32(lsa->id & ln->optx),
			u32_masklen(ln->optx));
...
static inline void show_lsa_sum_net(struct top_hash_entry *he, int ospf2)
{
	ip_addr ip;
	int pxlen;
	u8 pxopts;
...
static inline void show_lsa_sum_rt(struct top_hash_entry *he, int ospf2)
{
	u32 metric;
	u32 dst_rid;
	u32 options;
...
static inline void show_lsa_external(struct top_hash_entry *he, int ospf2)
{
	...
	cli_msg(-1016, "\t\t%s %I/%d metric%s %u%s%s",
		...
		rt.ip, rt.pxlen, rt.ebit ? "2" : "", rt.metric, str_via,
		str_tag);
}
...
show_lsa_prefix(struct top_hash_entry *he, struct top_hash_entry *cnode)
{
	...
	/* We check whether given prefix-LSA is related to the current struct list_head */
	if ((px->ref_type != cnode->lsa.type_raw)
	    || (px->ref_rt != cnode->lsa.rt))
		return;
	...
	for (i = 0; i < px->pxcount; i++) {
		...
		if (px->ref_type == LSA_T_RT)
			cli_msg(-1016, "\t\tstubnet %I/%d metric %u", pxa,
				pxlen, metric);
		else
...
void ospf_sh_state(struct proto *P, int verbose, int reachable)
{
	 ...
	 * currently opened struct list_head (whose header was presented). The LSAs are
	 ...
	 * the struct list_head is closed, related external routes are presented. We
	...
	for (i = 0; i < j1; i++) {
		...
		if (!cnode) {
			if (((he->lsa_type == LSA_T_RT)
			     || (he->lsa_type == LSA_T_NET))
		...
		/* In these cases, we close the current struct list_head */
		if ((i + 1 == j1)
...
static int lsa_compare_for_lsadb(const void *p1, const void *p2)
{
	struct top_hash_entry *he1 = *(struct top_hash_entry **)p1;
	struct top_hash_entry *he2 = *(struct top_hash_entry **)p2;
	struct ospf_lsa_header *lsa1 = &(he1->lsa);
...
{
	struct ospf_proto *p =
	    (struct ospf_proto *)proto_get_named(ld->name, &proto_ospf);
	...
	for (i = 0; i < j; i++) {
		...
		if ((ld->scope == LSA_SCOPE_AREA)
		    && (hea[i]->domain != ld->area))
			continue;
		...
		cli_msg(-1017, " %04x  %-15R %-15R  %08x %5u    %04x",
			lsa_type, lsa->id, lsa->rt, lsa->sn, lsa->age,
			lsa->checksum);
	}
	cli_msg(0, "");
}
