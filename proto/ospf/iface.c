...
static void poll_timer_hook(struct timer * timer)
{
	ospf_send_hello(timer->data, OHS_POLL, NULL);
}

static void hello_timer_hook(struct timer * timer)
{
	ospf_send_hello(timer->data, OHS_HELLO, NULL);
}

static void wait_timer_hook(struct timer * timer)
{
	struct ospf_iface *ifa = (struct ospf_iface *)timer->data;
	struct ospf_proto *p = ifa->oa->po;

	OSPF_TRACE(D_EVENTS, "Wait struct timer fired on %s", ifa->ifname);
	ospf_iface_sm(ifa, ISM_WAITF);
}

static inline uint ifa_tx_length(struct ospf_iface *ifa)
{
	return ifa->cf->tx_length ? : ifa->iface->mtu;
}

static inline uint ifa_bufsize(struct ospf_iface *ifa)
{
	uint bsize = ifa->cf->rx_buffer ? : ifa->iface->mtu;
	return MAX(bsize, ifa->tx_length);
}

static inline uint ifa_flood_queue_size(struct ospf_iface *ifa)
{
...
static int ospf_sk_open(struct ospf_iface *ifa)
{
	...
	struct birdsock *sk = sk_new(ifa->pool);
	sk->type = SK_IP;
	sk->dport = OSPF_PROTO;
...
static inline void ospf_sk_join_dr(struct ospf_iface *ifa)
{
	if (ifa->sk_dr)
		return;
...
static inline void ospf_sk_leave_dr(struct ospf_iface *ifa)
{
	if (!ifa->sk_dr)
		return;
...
static void ospf_iface_down(struct ospf_iface *ifa)
{
	struct ospf_proto *p = ifa->oa->po;
...
void ospf_iface_remove(struct ospf_iface *ifa)
{
	...
	if (ifa->type == OSPF_IT_VLINK)
		OSPF_TRACE(D_EVENTS, "Removing vlink to %R via area %R",
			   ifa->vid, ifa->voa->areaid);
...
void ospf_iface_chstate(struct ospf_iface *ifa, u8 state)
{
	...
	if ((ifa->type == OSPF_IT_BCAST) && ipa_nonzero(ifa->des_routers)
	    && ifa->sk) {
 ...
 * @ifa: OSPF interface
 * @event: struct event comming to state machine
 *
	...
	case ISM_UP:
		if (ifa->state <= OSPF_IS_LOOP) {
			...
			if ((ifa->type == OSPF_IT_PTP) ||
				...
				if (ifa->priority == 0)
					ospf_iface_chstate(ifa,
							   OSPF_IS_DROTHER);
				else {
					ospf_iface_chstate(ifa,
							   OSPF_IS_WAITING);
					if (ifa->wait_timer)
						tm_start(ifa->wait_timer,
							 ifa->waitint);
				}
			}
...
static u8 ospf_iface_classify_(struct iface *ifa, struct ifa *addr)
{
...
static inline u8 ospf_iface_classify(u8 type, struct ifa *addr)
{
	return (type !=
		OSPF_IT_UNDEF) ? type : ospf_iface_classify_(addr->iface, addr);
}
...
static void ospf_iface_add(struct object_lock *lock)
{
	...
	if (!ifa->stub) {
		ifa->hello_timer =
		    tm_new_set(ifa->pool, hello_timer_hook, ifa, 0,
			       ifa->helloint);

		if (ifa->type == OSPF_IT_NBMA)
			ifa->poll_timer =
			    tm_new_set(ifa->pool, poll_timer_hook, ifa, 0,
				       ifa->pollint);

		if ((ifa->type == OSPF_IT_BCAST) || (ifa->type == OSPF_IT_NBMA))
			ifa->wait_timer =
			    tm_new_set(ifa->pool, wait_timer_hook, ifa, 0, 0);

		ifa->flood_queue_size = ifa_flood_queue_size(ifa);
		ifa->flood_queue =
		    mb_allocz(ifa->pool,
			      ifa->flood_queue_size * sizeof(void *));
	}
...
static int ospf_iface_stubby(struct ospf_iface_patt *ip, struct ifa *addr)
{
	/* a host address */
...
void
ospf_iface_new(struct ospf_area *oa, struct ifa *addr,
	       struct ospf_iface_patt *ip)
{
	...
	if (ospf_is_v2(p) && (ifa->type == OSPF_IT_BCAST)
	    && (addr->flags & IA_PEER))
		ifa->type = OSPF_IT_PTP;

	if (ospf_is_v2(p) && (ifa->type == OSPF_IT_NBMA)
	    && (addr->flags & IA_PEER))
		ifa->type = OSPF_IT_PTMP;
	...
	if (ifa->type != old_type)
		log(L_WARN "%s: Cannot use interface %s as %s, forcing %s",
		    p->p.name, iface->name, ospf_it[old_type],
		    ospf_it[ifa->type]);
	...
	WALK_LIST(nb, ip->nbma_list) {
		...
		if (ospf_is_v2(p)
		    && !ipa_in_net(nb->ip, addr->prefix, addr->pxlen))
			continue;

		if (ospf_is_v3(p) && !ipa_is_link_local(nb->ip))
			log(L_WARN
			    "%s: Configured struct neighbor address (%I) should be link-local",
			    p->p.name, nb->ip);
...
void ospf_iface_new_vlink(struct ospf_proto *p, struct ospf_iface_patt *ip)
{
	...
	OSPF_TRACE(D_EVENTS, "Adding vlink to %R via area %R", ip->vid,
		   ip->voa);
	...
	ifa->flood_queue_size = ifa_flood_queue_size(ifa);
	ifa->flood_queue =
	    mb_allocz(ifa->pool, ifa->flood_queue_size * sizeof(void *));
}

static void ospf_iface_change_timer(struct timer * tm, uint val)
{
	if (!tm)
		return;
...
static inline void ospf_iface_update_flood_queue_size(struct ospf_iface *ifa)
{
	...
	ifa->flood_queue =
	    mb_realloc(ifa->flood_queue, new_size * sizeof(void *));
	bzero(ifa->flood_queue + old_size,
...
int ospf_iface_reconfigure(struct ospf_iface *ifa, struct ospf_iface_patt *new)
{
	...
	if (ifa->pollint != new->pollint) {
		OSPF_TRACE(D_EVENTS,
			   "Changing poll interval of %s from %d to %d", ifname,
	...
	if (ifa->waitint != new->waitint) {
		OSPF_TRACE(D_EVENTS,
			   "Changing wait interval of %s from %d to %d", ifname,
	...
	if (ifa->deadint != new->deadint) {
		OSPF_TRACE(D_EVENTS,
			   "Changing dead interval of %s from %d to %d", ifname,
	...
	if (ifa->autype != new->autype) {
		OSPF_TRACE(D_EVENTS, "Changing authentication type of %s",
			   ifname);
	...
	WALK_LIST_DELSAFE(nb, nbx, ifa->nbma_list) {
		...
		if (nb2) {
			if (nb->eligible != nb2->eligible) {
				OSPF_TRACE(D_EVENTS,
					   "Changing eligibility of NBMA struct neighbor %I on %s",
					   nb->ip, ifname);
	...
	WALK_LIST(nb, new->nbma_list) {
		...
		if (!find_nbma_node(ifa, nb->ip)) {
			OSPF_TRACE(D_EVENTS, "Adding NBMA struct neighbor %I on %s",
				   nb->ip, ifname);
			add_nbma_node(ifa, nb,
				      ! !find_neigh_by_ip(ifa, nb->ip));
		}
	}
	...
	if (old->rx_buffer != new->rx_buffer) {
		OSPF_TRACE(D_EVENTS, "Changing struct buffer size of %s from %d to %d",
	...
	if (((ifa->type == OSPF_IT_PTP) || (ifa->type == OSPF_IT_PTMP)) &&
		...
		OSPF_TRACE(D_EVENTS,
			   ...
			   ifname, ifa->link_lsa_suppression,
...
step:
	...
	if (s->warn)
		log(L_WARN "%s: Interface %s matches for multiple areas",
		    p->p.name, s->iface->name);

	return 0;
}

static struct ospf_iface *ospf_iface_find_by_key(struct ospf_proto *p,
						 struct ifa *a, int instance_id)
{
	struct ospf_iface *ifa;
...
static void ospf_reconfigure_ifaces2(struct ospf_proto *p)
{
	struct iface *iface;
	struct ifa *a;
	...
	WALK_LIST(iface, iface_list) {
		...
		WALK_LIST(a, iface->addrs) {
			...
			while (ospf_walk_matching_iface_patts(p, &s)) {
				/* Main inner loop */
				struct ospf_iface *ifa =
				    ospf_iface_find_by_key(p, a,
							   s.ip->instance_id);
				if (ifa) {
					if ((ifa->oa == s.oa)
					    && (ifa->marked < 2)
					    && ospf_iface_reconfigure(ifa,
								      s.ip))
						continue;
...
static void ospf_reconfigure_ifaces3(struct ospf_proto *p)
{
	struct iface *iface;
	struct ifa *a;
...
static void ospf_iface_change_mtu(struct ospf_proto *p, struct ospf_iface *ifa)
{
	/* ifa is not vlink */
...
void ospf_iface_info(struct ospf_iface *ifa)
{
	...
	if (ifa->type == OSPF_IT_VLINK) {
		...
		if (ospf_is_v3(ifa->oa->po))
			cli_msg(-1015, "Interface %s (IID %d)", ifa->ifname,
				ifa->instance_id);
		else if (ifa->addr->flags & IA_PEER)
			cli_msg(-1015, "Interface %s (peer %I)", ifa->ifname,
				ifa->addr->opposite);
		else
			cli_msg(-1015, "Interface %s (%I/%d)", ifa->ifname,
				ifa->addr->prefix, ifa->addr->pxlen);

		cli_msg(-1015, "\tType: %s%s", ospf_it[ifa->type], more);
		cli_msg(-1015, "\tArea: %R (%u)", ifa->oa->areaid,
			ifa->oa->areaid);
	}
	cli_msg(-1015, "\tState: %s%s", ospf_is_names[ifa->state],
		ifa->stub ? " (stub)" : "");
	cli_msg(-1015, "\tPriority: %u", ifa->priority);
	cli_msg(-1015, "\tCost: %u", ifa->cost);
	if (ifa->oa->po->ecmp)
		cli_msg(-1015, "\tECMP weight: %d",
			((int)ifa->ecmp_weight) + 1);
	...
	if ((ifa->type == OSPF_IT_BCAST) || (ifa->type == OSPF_IT_NBMA)) {
		...
		cli_msg(-1015, "\tBackup designated router (ID): %R",
			ifa->bdrid);
		cli_msg(-1015, "\tBackup designated router (IP): %I",
			ifa->bdrip);
	}
}
