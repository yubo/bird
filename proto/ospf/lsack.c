...
static void ospf_dump_lsack(struct ospf_proto *p, struct ospf_packet *pkt)
{
	struct ospf_lsa_header *lsas;
	uint i, lsa_count;
...
ospf_send_lsack_(struct ospf_proto *p, struct ospf_neighbor *n, int queue)
{
	...
	OSPF_PACKET(ospf_dump_lsack, pkt, "LSACK packet sent via %s",
		    ifa->ifname);
		   ...
		   struct ospf_neighbor *n)
{
	...
	OSPF_PACKET(ospf_dump_lsack, pkt,
		    "LSACK packet received from nbr %R on %s", n->rid,
		    ifa->ifname);
	...
	for (i = 0; i < lsa_count; i++) {
		...
		if (lsa_comp(&lsa, &ret->lsa) != CMP_SAME) {
			OSPF_TRACE(D_PACKETS, "Strange LSACK from nbr %R on %s",
				   n->rid, ifa->ifname);
			OSPF_TRACE(D_PACKETS, "    Type: %04x, Id: %R, Rt: %R",
				   lsa_type, lsa.id, lsa.rt);
			OSPF_TRACE(D_PACKETS,
				   "    I have: Seq: %08x, Age: %4u, Sum: %04x",
				   ret->lsa.sn, ret->lsa.age,
				   ret->lsa.checksum);
			OSPF_TRACE(D_PACKETS,
				   "    It has: Seq: %08x, Age: %4u, Sum: %04x",
				   lsa.sn, lsa.age, lsa.checksum);
			continue;
		}

		DBG("Deleting LSA (Type: %04x Id: %R Rt: %R) from lsrtl for struct neighbor %R\n", lsa_type, lsa.id, lsa.rt, n->rid);
