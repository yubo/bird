...
static int unknown_lsa_type(u32 type)
{
	switch (type) {
	case LSA_T_RT:
	case LSA_T_NET:
...
static const u16 lsa_v2_types[LSA_V2_TMAX] =
    { 0, LSA_T_RT, LSA_T_NET, LSA_T_SUM_NET, LSA_T_SUM_RT, LSA_T_EXT, 0,
LSA_T_NSSA };

void
lsa_get_type_domain_(u32 itype, struct ospf_iface *ifa, u32 * otype,
		     u32 * domain)
{
...
void lsa_generate_checksum(struct ospf_lsa_header *lsa, const u8 * body)
{
	...
	fletcher16_init(&ctx);
	fletcher16_update(&ctx, (u8 *) & hdr + 2,
			  sizeof(struct ospf_lsa_header) - 2);
	fletcher16_update_n32(&ctx, body, len - sizeof(struct ospf_lsa_header));
	lsa->checksum =
	    fletcher16_final(&ctx, len,
			     OFFSETOF(struct ospf_lsa_header, checksum));
}
...
static inline int lsa_walk_rt2(struct ospf_lsa_rt_walk *rt)
{
	if (rt->buf >= rt->bufend)
		return 0;
	...
	rt->buf +=
	    ...
	    l->no_tos * sizeof(struct ospf_lsa_rt2_tos);
...
static inline int lsa_walk_rt3(struct ospf_lsa_rt_walk *rt)
{
...
void
lsa_walk_rt_init(struct ospf_proto *p, struct top_hash_entry *act,
		 struct ospf_lsa_rt_walk *rt)
{
	...
	if (rt->ospf2)
		...
		rt->en =
		    ospf_hash_find_rt3_first(p->gr, act->domain, act->lsa.rt);

	rt->buf = rt->en->lsa_body;
	rt->bufend =
	    rt->buf + rt->en->lsa.length - sizeof(struct ospf_lsa_header);
	rt->buf += sizeof(struct ospf_lsa_rt);
}
...
void
lsa_parse_sum_net(struct top_hash_entry *en, int ospf2, ip_addr * ip,
		  int *pxlen, u8 * pxopts, u32 * metric)
{
	if (ospf2) {
...
void
lsa_parse_sum_rt(struct top_hash_entry *en, int ospf2, u32 * drid, u32 * metric,
		 u32 * options)
{
	if (ospf2) {
		struct ospf_lsa_sum2 *ls = en->lsa_body;
...
void
lsa_parse_ext(struct top_hash_entry *en, int ospf2,
	      struct ospf_lsa_ext_local *rt)
{
	if (ospf2) {
		...
		u16 rest;
		u32 *buf =
		    lsa_get_ipv6_prefix(ext->rest, &rt->ip, &rt->pxlen,
					&rt->pxopts, &rest);
		rt->metric = ext->metric & LSA_METRIC_MASK;
...
lsa_validate_rt2(struct ospf_lsa_header *lsa, struct ospf_lsa_rt *body)
{
	...
	while (buf < bufend) {
		...
		if (!((l->type == LSART_PTP) ||
		      (l->type == LSART_NET) ||
		      (l->type == LSART_STUB) || (l->type == LSART_VLNK)))
			return 0;
	}
...
lsa_validate_rt3(struct ospf_lsa_header *lsa, struct ospf_lsa_rt *body)
{
	...
	while (buf < bufend) {
		...
		if (!((l->type == LSART_PTP) ||
		      (l->type == LSART_NET) || (l->type == LSART_VLNK)))
			return 0;
	}
	return 1;
}
...
static inline int pxlen(u32 * buf)
{
	return *buf >> 24;
}

static int
lsa_validate_sum3_net(struct ospf_lsa_header *lsa,
		      struct ospf_lsa_sum3_net *body)
{
...
static int
lsa_validate_pxlist(struct ospf_lsa_header *lsa, u32 pxcount, uint offset,
		    u8 * pbuf)
{
...
lsa_validate_link(struct ospf_lsa_header *lsa, struct ospf_lsa_link *body)
{
	...
	return lsa_validate_pxlist(lsa, body->pxcount,
				   sizeof(struct ospf_lsa_link), (u8 *) body);
}
...
lsa_validate_prefix(struct ospf_lsa_header *lsa, struct ospf_lsa_prefix *body)
{
	...
	return lsa_validate_pxlist(lsa, body->pxcount,
				   sizeof(struct ospf_lsa_prefix), (u8 *) body);
}
