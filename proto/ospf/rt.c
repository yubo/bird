...
static inline void reset_ri(struct ort * ort)
{
	bzero(&ort->n, sizeof(struct orta));
}

void ospf_rt_initort(struct fib_node *fn)
{
	struct ort *ri = (struct ort *) fn;
	reset_ri(ri);
	ri->old_rta = NULL;
	ri->fn.flags = 0;
}

static inline int nh_is_vlink(struct mpnh *nhs)
{
	return !nhs->iface;
}

static inline int unresolved_vlink(struct ort * ort)
{
	return ort->n.nhs && nh_is_vlink(ort->n.nhs);
}

static inline struct mpnh *new_nexthop(struct ospf_proto *p, ip_addr gw,
				       struct iface *iface, byte weight)
{
...
static inline int has_device_nexthops(const struct mpnh *n)
{
	for (; n; n = n->next)
		if (ipa_zero(n->gw))
			return 1;
...
static struct mpnh *fix_device_nexthops(struct ospf_proto *p,
					const struct mpnh *n, ip_addr gw)
{
	...
	for (; n; n = n->next) {
		struct mpnh *nn =
		    new_nexthop(p, ipa_zero(n->gw) ? gw : n->gw, n->iface,
				n->weight);
...
static inline int epath_preferred(const struct orta * ep)
{
	return (ep->type == RTS_OSPF) && (ep->oa->areaid != 0);
}

/* Whether the ext route has ASBR/next_hop marked as preferred. */
static inline int orta_pref(const struct orta * nf)
{
	return ! !(nf->options & ORTA_PREF);
}

/* Classify struct orta entries according to RFC 3101 2.5 (6e) priorities:
   Type-7 LSA with P-bit, Type-5 LSA, Type-7 LSA without P-bit */
static int orta_prio(const struct orta * nf)
{
...
static int orta_prefer_lsa(const struct orta * new, const struct orta * old)
{
 ...
 * <, = or > than 0 if the new struct orta is less, equal or more preferred than
 * the old orta.
 */
static int
orta_compare(const struct ospf_proto *p, const struct orta * new, const struct orta * old)
{
	int r;
 ...
 */
static int
orta_compare_asbr(const struct ospf_proto *p, const struct orta * new,
		  const struct orta * old)
{
	int r;
 ...
 * than 0 if the new struct orta is less, equal or more preferred than the old orta.
 */
static int
orta_compare_ext(const struct ospf_proto *p, const struct orta * new, const struct orta * old)
{
	int r;
...
static inline void ort_replace(struct ort * o, const struct orta * new)
{
	memcpy(&o->n, new, sizeof(struct orta));
}

static void ort_merge(struct ospf_proto *p, struct ort * o, const struct orta * new)
{
	struct orta *old = &o->n;

	if (old->nhs != new->nhs) {
		old->nhs =
		    mpnh_merge(old->nhs, new->nhs, old->nhs_reuse,
			       new->nhs_reuse, p->ecmp, p->nhpool);
		old->nhs_reuse = 1;
	}
...
static void ort_merge_ext(struct ospf_proto *p, struct ort * o, const struct orta * new)
{
	struct orta *old = &o->n;
...
static inline void
ri_install_net(struct ospf_proto *p, ip_addr prefix, int pxlen,
	       const struct orta * new)
{
	struct ort *old = (struct ort *) fib_get(&p->rtf, &prefix, pxlen);
...
static inline void
ri_install_rt(struct ospf_area *oa, u32 rid, const struct orta * new)
{
	ip_addr addr = ipa_from_rid(rid);
	struct ort *old = (struct ort *) fib_get(&oa->rtr, &addr, MAX_PREFIX_LENGTH);
...
static inline void
ri_install_asbr(struct ospf_proto *p, ip_addr * addr, const struct orta * new)
{
	struct ort *old = (struct ort *) fib_get(&p->backbone->rtr, addr, MAX_PREFIX_LENGTH);
	if (orta_compare_asbr(p, new, &old->n) > 0)
		ort_replace(old, new);
}

static inline void
ri_install_ext(struct ospf_proto *p, ip_addr prefix, int pxlen,
	       const struct orta * new)
{
	struct ort *old = (struct ort *) fib_get(&p->rtf, &prefix, pxlen);
...
static inline struct ospf_iface *rt_pos_to_ifa(struct ospf_area *oa, int pos)
{
...
static inline struct ospf_iface *px_pos_to_ifa(struct ospf_area *oa, int pos)
{
	struct ospf_iface *ifa;
...
static void
add_network(struct ospf_area *oa, ip_addr px, int pxlen, int metric,
...
static inline void
spfa_process_rt(struct ospf_proto *p, struct ospf_area *oa,
		struct top_hash_entry *act)
{
	...
	for (lsa_walk_rt_init(p, act, &rtl), i = 0; lsa_walk_rt(&rtl); i++) {
		...
		case LSART_STUB:
			...
			add_network(oa, prefix, pxlen, act->dist + rtl.metric,
				    act, i);
			break;

		case LSART_NET:
			tmp =
			    ospf_hash_find_net(p->gr, oa->areaid, rtl.id,
					       rtl.nif);
			break;
...
static inline void
spfa_process_net(struct ospf_proto *p, struct ospf_area *oa,
		 struct top_hash_entry *act)
{
...
spfa_process_prefixes(struct ospf_proto *p, struct ospf_area *oa)
{
	...
	WALK_SLIST(en, p->lsal) {
		...
		if (px->ref_type == LSA_T_RT)
			...
			src =
			    ospf_hash_find(p->gr, oa->areaid, px->ref_id,
		...
		for (i = 0; i < px->pxcount; i++) {
			buf =
			    lsa_get_ipv6_prefix(buf, &pxa, &pxlen, &pxopts,
						&metric);
...
static void ospf_rt_spfa(struct ospf_area *oa)
{
	struct ospf_proto *p = oa->po;
	struct top_hash_entry *act;
	struct node *n;
	...
	OSPF_TRACE(D_EVENTS, "Starting routing table calculation for area %R",
		   oa->areaid);
	...
	while (!EMPTY_LIST(oa->cand)) {
		...
		default:
			log(L_WARN "%s: Unknown LSA type in SPF: %d", p->p.name,
			    act->lsa_type);
		}
	}
	  ...
	  struct top_hash_entry *par)
{
	...
	case LSA_T_RT:
		...
		while (lsa_walk_rt(&rtl)) {
			...
			case LSART_NET:
				tmp =
				    ospf_hash_find_net(p->gr, oa->areaid,
						       rtl.id, rtl.nif);
				if (tmp == par) {
					if (ospf_is_v2(p))
			...
			case LSART_PTP:
				...
				tmp =
				    ospf_hash_find_rt(p->gr, oa->areaid,
						      rtl.id);
				if (tmp == par)
					return 1;
				break;
...
static void ospf_rt_sum(struct ospf_area *oa)
{
	struct ospf_proto *p = oa->po;
	...
	WALK_SLIST(en, p->lsal) {
		...
		if (en->lsa_type == LSA_T_SUM_NET) {
			lsa_parse_sum_net(en, ospf_is_v2(p), &ip, &pxlen,
					  &pxopts, &metric);
			...
			if (pxlen < 0 || pxlen > MAX_PREFIX_LENGTH) {
				log(L_WARN
				    "%s: Invalid prefix in LSA (Type: %04x, Id: %R, Rt: %R)",
				    p->p.name, en->lsa_type, en->lsa.id,
				    en->lsa.rt);
				continue;
			}
			...
			lsa_parse_sum_rt(en, ospf_is_v2(p), &dst_rid, &metric,
					 &options);
		...
		abr = (struct ort *) fib_find(&oa->rtr, &abrip, MAX_PREFIX_LENGTH);
		if (!abr || !abr->n.type)
			continue;
...
static void ospf_rt_sum_tr(struct ospf_area *oa)
{
	struct ospf_proto *p = oa->po;
...
static int
decide_anet_lsa(struct ospf_area *oa, struct area_net *anet,
...
static int decide_sum_lsa(struct ospf_area *oa, struct ort * nf, int dest)
{
	/* 12.4.3.1. - for stub/NSSA areas, originating summary routes is configurable */
	if (!oa_is_ext(oa) && !oa->ac->summary)
		return 0;
...
static inline void check_sum_net_lsa(struct ospf_proto *p, struct ort * nf)
{
...
static inline int
decide_nssa_lsa(struct ospf_proto *p, struct ort * nf, struct ospf_lsa_ext_local *rt)
{
...
static inline void check_nssa_lsa(struct ospf_proto *p, struct ort * nf)
{
	struct area_net *anet = NULL;
	...
	if (anet && anet->active && !anet->hidden && oa->translate)
		ospf_originate_ext_lsa(p, NULL, nf, LSA_M_RTCALC, anet->metric,
				       (anet->metric & LSA_EXT3_EBIT), IPA_NONE,
				       anet->tag, 0);

	/* RFC 3103 3.2 (2) - originate the same network */
	else if (decide_nssa_lsa(p, nf, &rt))
		ospf_originate_ext_lsa(p, NULL, nf, LSA_M_RTCALC, rt.metric,
				       rt.ebit, rt.fwaddr, rt.tag, 0);
}
...
static void ospf_check_vlinks(struct ospf_proto *p)
{
	...
	WALK_LIST(ifa, p->iface_list) {
		if (ifa->type == OSPF_IT_VLINK) {
			struct top_hash_entry *tmp;
			tmp =
			    ospf_hash_find_rt(p->gr, ifa->voa->areaid,
					      ifa->vid);

			if (tmp && (tmp->color == INSPF) && ipa_nonzero(tmp->lb)
			    && tmp->nhs) {
				struct ospf_iface *nhi =
				    ospf_iface_find(p, tmp->nhs->iface);
...
static void ospf_rt_abr1(struct ospf_proto *p)
{
	struct area_net *anet;
	struct ort *nf, *default_nf;
	...
	FIB_WALK(&p->rtf, nftmp) {
		...
		if (nf->n.type == RTS_OSPF) {
			anet =
			    (struct area_net *)fib_route(&nf->n.oa->net_fib,
							 nf->fn.prefix,
							 nf->fn.pxlen);
			if (anet) {
				if (!anet->active) {
	...
	default_nf = (struct ort *) fib_get(&p->rtf, &addr, 0);
	...
	WALK_LIST(oa, p->area_list) {
		...
		if (oa_is_stub(oa) || (oa_is_nssa(oa) && !oa->ac->summary))
			ospf_originate_sum_net_lsa(p, oa, default_nf,
						   oa->ac->default_cost);
		...
		if (oa_is_nssa(oa) && oa->ac->default_nssa)
			ospf_originate_ext_lsa(p, oa, default_nf, LSA_M_RTCALC,
					       oa->ac->default_cost,
					       (oa->ac->
						default_cost & LSA_EXT3_EBIT),
					       IPA_NONE, 0, 0);
		...
		if (oa_is_ext(oa)) {
			FIB_WALK(&oa->rtr, nftmp) {
				...
				if (nf->n.options & ORTA_ASBR)
					ri_install_asbr(p, &nf->fn.prefix,
							&nf->n);
			}
			FIB_WALK_END;
		}
	...
	FIB_WALK(&p->backbone->rtr, nftmp) {
		check_sum_rt_lsa(p, (struct ort *) nftmp);
	}
	FIB_WALK_END;
...
static void translator_timer_hook(struct timer * timer)
{
...
static void ospf_rt_abr2(struct ospf_proto *p)
{
	struct ospf_area *oa;
	struct top_hash_entry *en;
	struct ort *nf, *nf2;
	...
	WALK_LIST(oa, p->area_list)
		...
		FIB_WALK(&oa->rtr, nftmp) {
			...
			nf2 =
			    fib_find(&bb->rtr, &nf->fn.prefix,
				     MAX_PREFIX_LENGTH);
			if (!nf2 || !nf2->n.type
			    || !(nf2->n.options & ORTA_ABR))
				continue;
			...
			if ((rt->options & OPT_RT_NT)
			    || (p->router_id < nf->n.rid)) {
				translate = 0;
				goto decided;
			}
		}
...
decided:
		...
		if (!translate && (oa->translate == TRANS_ON)) {
			if (oa->translator_timer == NULL)
				oa->translator_timer =
				    tm_new_set(p->p.pool, translator_timer_hook,
					       oa, 0, 0);
	...
	FIB_WALK(&p->rtf, nftmp) {
		...
		if (rt_is_nssa(nf) && (nf->n.options & ORTA_PROP)) {
			struct area_net *anet = (struct area_net *)
			    fib_route(&nf->n.oa->enet_fib, nf->fn.prefix,
				      nf->fn.pxlen);
...
static void *ospf_fib_route(struct fib *f, ip_addr a, int len)
{
	ip_addr a0;
	struct ort *nf;
...
static void ospf_ext_spf(struct ospf_proto *p)
{
	struct top_hash_entry *en;
	struct ospf_lsa_ext_local rt;
	struct ort *nf1, *nf2;
	...
	WALK_SLIST(en, p->lsal) {
		...
		if ((en->lsa_type == LSA_T_NSSA) && ipa_zero(rt.ip)
		    && (rt.pxlen == 0) && (p->areano > 1) && !(rt.propagate
							       && atmp->ac->
							       summary))
			continue;
		...
		if (!rt.fbit) {
			...
			nf2 =
			    ospf_fib_route(&p->rtf, rt.fwaddr,
					   MAX_PREFIX_LENGTH);
			if (!nf2)
				continue;
			...
			if (en->lsa_type == LSA_T_EXT) {
				...
				if ((nf2->n.type != RTS_OSPF)
				    && (nf2->n.type != RTS_OSPF_IA))
					continue;
			} else {	/* NSSA */
			...
			if (has_device_nexthops(nfa.nhs)) {
				nfa.nhs =
				    fix_device_nexthops(p, nfa.nhs, rt.fwaddr);
				nfa.nhs_reuse = 1;
			}
		}
...
void ospf_rt_reset(struct ospf_proto *p)
{
	struct ospf_area *oa;
	struct top_hash_entry *en;
	struct area_net *anet;
				  ...
				  struct top_hash_entry *par, int pos)
{
	 ...
	 * 2) en is a ptp or ptmp struct neighbor (and par is root)
	 * 3) en is a bcast or nbma struct neighbor (and par is local network)
	 */
	...
	/* The second case - ptp or ptmp struct neighbor */
	...
	/* The third case - bcast or nbma struct neighbor */
	if ((en->lsa_type == LSA_T_RT) && (par->lsa_type == LSA_T_NET)) {
		...
		if (ospf_is_v2(p)) {
			...
			lhe =
			    ospf_hash_find(p->gr, pn->iface->index, en->lb_id,
					   rid, LSA_T_LINK);
			...
			return new_nexthop(p, ipa_from_ip6(llsa->lladdr),
					   pn->iface, pn->weight);
		}
	}
	 ...
	 u32 dist, struct ospf_area *oa, int pos)
{
	...
	if ((dist == en->dist) && !nh_is_vlink(en->nhs)) {
		...
		en->nhs =
		    mpnh_merge(en->nhs, nhs, en->nhs_reuse, new_reuse, p->ecmp,
			       p->nhpool);
		en->nhs_reuse = 1;
		return;
	}
...
static inline int ort_changed(struct ort * nf, struct rta * nr)
{
	struct rta *or = nf->old_rta;
	return !or ||
	    (nf->n.metric1 != nf->old_metric1)
	    || (nf->n.metric2 != nf->old_metric2) || (nf->n.tag != nf->old_tag)
	    || (nf->n.rid != nf->old_rid) || (nr->source != or->source)
	    || (nr->dest != or->dest) || (nr->iface != or->iface)
	    || !ipa_equal(nr->gw, or->gw)
	...
	FIB_ITERATE_START(fib, &fit, nftmp) {
		...
		if (nf->n.type) {	/* Add the route */
			...
			if (reload || ort_changed(nf, &a0)) {
				...
				e->u.ospf.metric1 = nf->old_metric1 =
				    nf->n.metric1;
				e->u.ospf.metric2 = nf->old_metric2 =
				    nf->n.metric2;
