		   ...
		   struct ospf_neighbor *n, ip_addr faddr)
{
	 ...
	 * packet instead of one from the struct neighbor structure for log messages.
	 */
	u32 rcv_rid = ntohl(pkt->routerid);
	OSPF_TRACE(D_PACKETS, "HELLO packet received from nbr %R on %s",
		   rcv_rid, ifa->ifname);
	...
	if (ospf_is_v2(p)) {
		...
		if ((ifa->type != OSPF_IT_VLINK) &&
		    (ifa->type != OSPF_IT_PTP) && (pxlen != ifa->addr->pxlen))
			DROP("prefix length mismatch", pxlen);

		neighbors = ps->neighbors;
		neigh_count =
		    (plen - sizeof(struct ospf_hello2_packet)) / sizeof(u32);
	} else {		/* OSPFv3 */
		...
		neighbors = ps->neighbors;
		neigh_count =
		    (plen - sizeof(struct ospf_hello3_packet)) / sizeof(u32);
	}
	...
	/* Check consistency of existing struct neighbor entry */
	if (n) {
		uint t = ifa->type;
		if (ospf_is_v2(p)
	...
	if (!n) {
		if ((ifa->type == OSPF_IT_NBMA) || (ifa->type == OSPF_IT_PTMP)) {
			...
			if (!nn && ifa->strictnbma)
				DROP1("new struct neighbor denied");
		...
		OSPF_TRACE(D_EVENTS, "New struct neighbor %R on %s, IP address %I",
...
OSPF_TRACE(D_EVENTS, "-----neighbor %x lsrt_timer %x", n,  n->lsrt_timer);
	...
	/* Update inactivity struct timer */
...
found_self:
	...
	if (n->iface_id != old_iface_id) {
		/* If struct neighbor is DR, also update cached DR interface ID */
		if (ifa->drid == n->rid)
