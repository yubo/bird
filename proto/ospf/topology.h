...
struct top_graph {
	struct pool *pool;		/* Pool we allocate from */
	struct slab *hash_slab;	/* Slab for hash entries */
...
void ospf_advance_lsa(struct ospf_proto *p, struct top_hash_entry *en,
		      struct ospf_lsa_header *lsa, u32 type, u32 domain,
		      void *body);
void ospf_flush_lsa(struct ospf_proto *p, struct top_hash_entry *en);
void ospf_update_lsadb(struct ospf_proto *p);

static inline void ospf_flush2_lsa(struct ospf_proto *p,
...
void ospf_originate_sum_net_lsa(struct ospf_proto *p, struct ospf_area *oa,
				struct ort * nf, int metric);
void ospf_originate_sum_rt_lsa(struct ospf_proto *p, struct ospf_area *oa,
			       struct ort * nf, int metric, u32 options);
void ospf_originate_ext_lsa(struct ospf_proto *p, struct ospf_area *oa,
...
struct top_hash_entry *ospf_hash_find(struct top_graph *, u32 domain, u32 lsa,
				      u32 rtr, u32 type);
struct top_hash_entry *ospf_hash_get(struct top_graph *, u32 domain, u32 lsa,
				     u32 rtr, u32 type);
...
static inline struct top_hash_entry *ospf_hash_get_entry(struct top_graph *f,
							 struct top_hash_entry
							 *en)
{
	return ospf_hash_get(f, en->domain, en->lsa.id, en->lsa.rt,
			     en->lsa_type);
}
...
/* In OSPFv2, id is network IP prefix (lsa.id) while lsa.rt field is unknown
   In OSPFv3, id is lsa.rt of DR while nif is struct neighbor iface id (lsa.id) */
static inline struct top_hash_entry *ospf_hash_find_net(struct top_graph *f,
							u32 domain, u32 id,
							u32 nif)
{
	return f->ospf2 ?
