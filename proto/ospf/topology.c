 ...
 */
struct top_hash_entry *ospf_install_lsa(struct ospf_proto *p,
					struct ospf_lsa_header *lsa, u32 type,
					u32 domain, void *body)
{
	...
	if ((en->lsa_body == NULL) ||	/* No old LSA */
	    ...
	    (lsa->age == LSA_MAXAGE) ||
	    memcmp(en->lsa_body, body,
		   lsa->length - sizeof(struct ospf_lsa_header)))
		change = 1;
 ...
 */
void
ospf_advance_lsa(struct ospf_proto *p, struct top_hash_entry *en,
		 struct ospf_lsa_header *lsa, u32 type, u32 domain, void *body)
{
	/* RFC 2328 13.4 */
	...
	if (en && (en->lsa.age < LSA_MAXAGE)) {
		if (lsa->sn != LSA_MAXSEQNO) {
			...
			if (en->next_lsa_body == NULL) {
				...
				en->next_lsa_opts =
				    ospf_is_v2(p) ? lsa_get_options(&en->
								    lsa) : 0;
			} else {
	 ...
	 * the struct neighbor we received it from), we cheat a bit here.
	 */
...
static int
ospf_do_originate_lsa(struct ospf_proto *p, struct top_hash_entry *en,
		      void *lsa_body, u16 lsa_blen, u16 lsa_opts)
{
	/* Enforce MinLSInterval */
	if ((en->init_age == 0) && en->inst_time
	    && ((en->inst_time + MINLSINTERVAL) > now))
		return 0;
 ...
 * specification (@lsa) and the LSA body from lsab struct buffer of @p, which is
 * emptied after the call. The opposite of this function is ospf_flush_lsa().
 */
struct top_hash_entry *ospf_originate_lsa(struct ospf_proto *p,
...
ospf_originate_next_lsa(struct ospf_proto *p, struct top_hash_entry *en)
{
	...
	if (!ospf_do_originate_lsa
	    (p, en, en->next_lsa_body, en->next_lsa_blen, en->next_lsa_opts))
		return;
...
static void ospf_refresh_lsa(struct ospf_proto *p, struct top_hash_entry *en)
{
	/*
	...
	if (en->lsa.sn == LSA_MAXSEQNO) {
		...
		en->next_lsa_opts =
		    ospf_is_v2(p) ? lsa_get_options(&en->lsa) : 0;
...
static void ospf_clear_lsa(struct ospf_proto *p, struct top_hash_entry *en)
{
	/*
...
static void ospf_remove_lsa(struct ospf_proto *p, struct top_hash_entry *en)
{
	/*
	 * Called by ospf_update_lsadb() as part of LSA flushing process.
...
void ospf_update_lsadb(struct ospf_proto *p)
{
	...
	list_for_each_safe(_p, _n, &p->lsal.n) {
		...
		if (en->lsa.age == LSA_MAXAGE) {
			if (en->lsa_body && (p->padj == 0)
			    && (en->ret_count == 0))
				ospf_clear_lsa(p, en);

			if ((en->lsa_body == NULL)
			    && (en->next_lsa_body == NULL)
			    && ((en->lsa.rt != p->router_id)
				|| (real_age >= LSA_MAXAGE)))
...
static inline u32 ort_to_lsaid(struct ospf_proto *p, struct ort * nf)
{
	/*
...
static void *lsab_alloc(struct ospf_proto *p, uint size)
{
...
static inline void *lsab_allocz(struct ospf_proto *p, uint size)
{
...
static inline void *lsab_offset(struct ospf_proto *p, uint offset)
{
	return ((byte *) p->lsab) + offset;
}

static inline void *lsab_end(struct ospf_proto *p)
{
...
static int configured_stubnet(struct ospf_area *oa, struct ifa *a)
{
	...
	list_for_each_entry(sn, &oa->ac->stubnet_list, n) {
		if (sn->summary) {
			...
			if (ipa_equal(a->prefix, sn->px.addr)
			    && (a->pxlen == sn->px.len))
				return 1;
		}
	}
...
static int bcast_net_active(struct ospf_iface *ifa)
{
...
{
	struct ospf_lsa_rt2_link *ln =
	    lsab_alloc(p, sizeof(struct ospf_lsa_rt2_link));
	ln->type = type;
	ln->id = id;
	ln->data = data;
	ln->metric = metric;
...
static void prepare_rt2_lsa_body(struct ospf_proto *p, struct ospf_area *oa)
{
	...
	list_for_each_entry(ifa, &p->iface_list, n) {
		...
		if ((ifa->addr->flags & IA_HOST) ||
		    (ifa->state == OSPF_IS_LOOP) || (ifa->type == OSPF_IT_PTMP))
			add_rt2_lsa_link(p, LSART_STUB,
					 ipa_to_u32(ifa->addr->ip), 0xffffffff,
					 0);
		else
			add_rt2_lsa_link(p, LSART_STUB,
					 ipa_to_u32(ifa->addr->prefix),
					 u32_mkmask(ifa->addr->pxlen),
					 ifa->cost);
		i++;
...
static inline void
add_rt3_lsa_link(struct ospf_proto *p, u8 type, struct ospf_iface *ifa, u32 nif,
		 u32 id)
{
	struct ospf_lsa_rt3_link *ln =
	    lsab_alloc(p, sizeof(struct ospf_lsa_rt3_link));
	ln->type = type;
	ln->padding = 0;
	ln->metric = ifa->cost;
	ln->lif = ifa->iface_id;
...
static void prepare_rt3_lsa_body(struct ospf_proto *p, struct ospf_area *oa)
{
	struct ospf_iface *ifa;
	struct ospf_neighbor *neigh;
	int bitv = 0;
	...
	list_for_each_entry(ifa, &p->iface_list, n) {
		...
		case OSPF_IT_NBMA:
			if (bcast_net_active(ifa))
				add_rt3_lsa_link(p, LSART_NET, ifa,
						 ifa->dr_iface_id, ifa->drid),
				    i++;
			break;
	...
	struct ospf_lsa_rt *rt = p->lsab;
	rt->options =
	    get_rt_options(p, oa, bitv) | (oa->options & LSA_OPTIONS_MASK);
}

static void ospf_originate_rt_lsa(struct ospf_proto *p, struct ospf_area *oa)
{
	struct ospf_new_lsa lsa = {
		.type = LSA_T_RT,
		.dom = oa->areaid,
...
static void prepare_net2_lsa_body(struct ospf_proto *p, struct ospf_iface *ifa)
{
...
static void prepare_net3_lsa_body(struct ospf_proto *p, struct ospf_iface *ifa)
{
	struct ospf_lsa_net *net;
	int nodes = ifa->fadj + 1;
	u32 options = 0;
	...
	list_for_each_entry(n, &ifa->neigh_list, n) {
		if (n->state == NEIGHBOR_FULL) {
			...
			struct top_hash_entry *en =
			    ospf_hash_find(p->gr, ifa->iface_id, n->iface_id,
					   n->rid, LSA_T_LINK);

			if (en)
				options |=
				    ((struct ospf_lsa_link *)en->lsa_body)->
				    options;
...
static void ospf_originate_net_lsa(struct ospf_proto *p, struct ospf_iface *ifa)
{
	struct ospf_new_lsa lsa = {
		.type = LSA_T_NET,
		.dom = ifa->oa->areaid,
	...
	OSPF_TRACE(D_EVENTS, "Updating network state for %s (Id: %R)",
		   ifa->ifname, lsa.id);
...
static inline void
prepare_sum3_net_lsa_body(struct ospf_proto *p, struct ort * nf, u32 metric)
{
...
static inline void
prepare_sum3_rt_lsa_body(struct ospf_proto *p, u32 drid, u32 metric,
			 u32 options)
{
...
void
ospf_originate_sum_net_lsa(struct ospf_proto *p, struct ospf_area *oa, struct ort * nf,
			   int metric)
{
	struct ospf_new_lsa lsa = {
		.type = LSA_T_SUM_NET,
...
void
ospf_originate_sum_rt_lsa(struct ospf_proto *p, struct ospf_area *oa, struct ort * nf,
			  int metric, u32 options)
{
	struct ospf_new_lsa lsa = {
		.type = LSA_T_SUM_RT,
		.mode = LSA_M_RTCALC,
	...
	if (ospf_is_v2(p))
		...
		prepare_sum3_rt_lsa_body(p, lsa.id, metric,
					 options & LSA_OPTIONS_MASK);
...
static inline void
prepare_ext3_lsa_body(struct ospf_proto *p, struct ort * nf,
		      u32 metric, u32 ebit, ip_addr fwaddr, u32 tag, int pbit)
{
	...
	buf =
	    put_ipv6_prefix(buf, nf->fn.prefix, nf->fn.pxlen,
			    pbit ? OPT_PX_P : 0, 0);
 ...
 */
void
ospf_originate_ext_lsa(struct ospf_proto *p, struct ospf_area *oa, struct ort * nf,
		       u8 mode, u32 metric, u32 ebit, ip_addr fwaddr, u32 tag,
		       int pbit)
{
	struct ospf_new_lsa lsa = {
	...
	if (ospf_is_v2(p))
		prepare_ext2_lsa_body(p, nf->fn.pxlen, metric, ebit, fwaddr,
				      tag);
	else
		prepare_ext3_lsa_body(p, nf, metric, ebit, fwaddr, tag, oa
				      && pbit);

	ospf_originate_lsa(p, &lsa);
}

static struct top_hash_entry *ospf_hash_find_(struct top_graph *f, u32 domain,
					      u32 lsa, u32 rtr, u32 type);

static void
ospf_flush_ext_lsa(struct ospf_proto *p, struct ospf_area *oa, struct ort * nf)
{
	struct top_hash_entry *en;
	       ...
	       struct rte * old UNUSED, struct ea_list * ea)
{
	...
	if (oa && ipa_zero(fwd)) {
		...
		if (ipa_zero(fwd)) {
			log(L_ERR
			    "%s: Cannot find forwarding address for NSSA-LSA %I/%d",
			    p->p.name, n->n.prefix, n->n.pxlen);
			return;
		}
	}

	nf = (struct ort *) fib_get(&p->rtf, &n->n.prefix, n->n.pxlen);
	ospf_originate_ext_lsa(p, oa, nf, LSA_M_EXPORT, metric, ebit, fwd, tag,
			       1);
	nf->external_rte = 1;
}
...
static void prepare_link_lsa_body(struct ospf_proto *p, struct ospf_iface *ifa)
{
...
ospf_originate_link_lsa(struct ospf_proto *p, struct ospf_iface *ifa)
{
	...
	OSPF_TRACE(D_EVENTS, "Updating link state for %s (Id: %R)", ifa->ifname,
		   lsa.id);
	...
	list_for_each_entry(ifa, &p->iface_list, n) {
		...
		list_for_each_entry(a, &ifa->iface->addrs, n) {
			...
			if ((a->flags & IA_HOST) ||
				...
				lsab_put_prefix(p, a->prefix, a->pxlen,
						ifa->cost);
			i++;
		}
...
static inline int prefix_space(u32 * buf)
{
	int pxl = *buf >> 24;
	return IPV6_PREFIX_SPACE(pxl);
}

static inline int prefix_same(u32 * b1, u32 * b2)
{
	int pxl1 = *b1 >> 24;
	int pxl2 = *b2 >> 24;
	int pxs, i;
...
static inline u32 *prefix_advance(u32 * buf)
{
	int pxl = *buf >> 24;
...
static void add_prefix(struct ospf_proto *p, u32 * px, int offset, int *pxc)
{
...
static void
add_link_lsa(struct ospf_proto *p, struct ospf_lsa_link *ll, int offset,
	     int *pxc)
{
	u32 *pxb = ll->rest;
	int j;
...
{
	return en && (en->lsa.age < LSA_MAXAGE)
	    && ((en->inst_time + MINLSINTERVAL) > now);
}
...
static void ospf_top_ht_alloc(struct top_graph *f)
{
...
static inline void ospf_top_ht_free(struct top_hash_entry **h)
{
	mb_free(h);
}

static inline u32 ospf_top_hash_u32(u32 a)
{
 ...
 * @p: OSPF protocol instance
 * @pool: struct pool for allocation
 *
 ...
 */
struct top_graph *ospf_top_new(struct ospf_proto *p, struct pool * pool)
{
	struct top_graph *f;
...
static void ospf_top_rehash(struct top_graph *f, int step)
{
	...
	for (oldh = 0; oldh < oldn; oldh++) {
		...
		while (e) {
			x = e->next;
			n = newt + ospf_top_hash(f, e->domain, e->lsa.id,
						 e->lsa.rt, e->lsa_type);
			e->next = *n;
			*n = e;
			e = x;
...
struct top_hash_entry *ospf_hash_find(struct top_graph *f, u32 domain, u32 lsa,
				      u32 rtr, u32 type)
{
...
struct top_hash_entry *ospf_hash_find_rt(struct top_graph *f, u32 domain,
					 u32 rtr)
{
	...
	while (e) {
		if (e->lsa.rt == rtr && e->lsa_type == LSA_T_RT
		    && e->domain == domain && e->lsa_body) {
 ...
 */
static inline struct top_hash_entry *find_matching_rt3(struct top_hash_entry *e,
						       u32 domain, u32 rtr)
{
...
struct top_hash_entry *ospf_hash_find_rt3_first(struct top_graph *f, u32 domain,
						u32 rtr)
{
	struct top_hash_entry *e;
...
struct top_hash_entry *ospf_hash_find_net2(struct top_graph *f, u32 domain,
					   u32 id)
{
	struct top_hash_entry *e;
...
struct top_hash_entry *ospf_hash_get(struct top_graph *f, u32 domain, u32 lsa,
				     u32 rtr, u32 type)
{
...
void ospf_hash_delete(struct top_graph *f, struct top_hash_entry *e)
{
