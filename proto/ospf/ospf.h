...
#define DEADC_D 4
#define WAIT_DMH 4
  /* Value of Wait struct timer - not found it in RFC * - using 4*HELLO */
...
struct ospf_proto {
	...
	struct event *flood_event;	/* Event for flooding LS updates */
	void *lsab;		/* LSA struct buffer used when originating router LSAs */
	int lsab_size, lsab_used;
	struct linpool *nhpool;	/* Linpool used for next hops computed in SPF */
	struct birdsock *vlink_sk;		/* IP socket used for vlink TX */
	u32 router_id;
...
struct ospf_area {
	...
	struct timer *translator_timer;	/* For NSSA translator switch */
...
struct ospf_iface {
	...
	struct timer *wait_timer;	/* WAIT struct timer */
	struct timer *hello_timer;	/* HELLOINT struct timer */
	struct timer *poll_timer;	/* Poll Interval - for NBMA */
...
struct ospf_neighbor {
	...
	u8 state;
	struct timer *inactim;		/* Inactivity struct timer */
...
#define OSPF_IS_WAITING	2	/* Waiting for Wait struct timer */
...
#define ISM_WAITF	1	/* Wait struct timer fired */
...
/* Definitions for struct neighbor state machine */
...
#define INM_INACTTIM	11	/* Inactivity struct timer */
...
static inline uint lsa_net_count(struct ospf_lsa_header *lsa)
{
	return (lsa->length - sizeof(struct ospf_lsa_header) -
...
static inline u32 *lsa_get_ipv6_prefix(u32 * buf, ip_addr * addr, int *pxlen,
				       u8 * pxopts, u16 * rest)
{
	u8 pxl = (*buf >> 24);
	*pxopts = (*buf >> 16);
...
static inline u32 *lsa_get_ipv6_addr(u32 * buf, ip_addr * addr)
{
	*addr = *(ip_addr *) buf;
	return buf + 4;
}

static inline u32 *put_ipv6_prefix(u32 * buf, ip_addr addr, u8 pxlen, u8 pxopts,
				   u16 lh)
{
#ifdef IPV6
...
static inline u32 *put_ipv6_addr(u32 * buf, ip_addr addr)
{
	*(ip_addr *) buf = addr;
	return buf + 4;
}
...
void ospf_iface_new(struct ospf_area *oa, struct ifa *addr,
...
int ospf_rx_hook(struct birdsock * sk, int size);
// void ospf_tx_hook(struct birdsock * sk);
...
{
	return ospf_is_v2(p) ? (sizeof(struct ospf_packet) +
				sizeof(union ospf_auth)) : sizeof(struct
								  ospf_packet);
}
...
void ospf_send_hello(struct ospf_iface *ifa, int kind,
		     struct ospf_neighbor *dirn);
void ospf_receive_hello(struct ospf_packet *pkt, struct ospf_iface *ifa,
...
void ospf_receive_dbdes(struct ospf_packet *pkt, struct ospf_iface *ifa,
			struct ospf_neighbor *n);

/* lsreq.c */
void ospf_send_lsreq(struct ospf_proto *p, struct ospf_neighbor *n);
void ospf_receive_lsreq(struct ospf_packet *pkt, struct ospf_iface *ifa,
			struct ospf_neighbor *n);
...
void ospf_lsa_lsrt_down_(struct top_hash_entry *en, struct ospf_neighbor *n,
...
int ospf_flood_lsa(struct ospf_proto *p, struct top_hash_entry *en,
		   struct ospf_neighbor *from);
int ospf_send_lsupd(struct ospf_proto *p, struct top_hash_entry **lsa_list,
		    uint lsa_count, struct ospf_neighbor *n);
void ospf_rxmt_lsupd(struct ospf_proto *p, struct ospf_neighbor *n);
void ospf_receive_lsupd(struct ospf_packet *pkt, struct ospf_iface *ifa,
			struct ospf_neighbor *n);

/* lsack.c */
void ospf_enqueue_lsack(struct ospf_neighbor *n, struct ospf_lsa_header *h_n,
			int queue);
...
void ospf_receive_lsack(struct ospf_packet *pkt, struct ospf_iface *ifa,
			struct ospf_neighbor *n);
