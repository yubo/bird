...
static inline uint ospf_dbdes_hdrlen(struct ospf_proto *p)
{
	return ospf_is_v2(p) ?
	    sizeof(struct ospf_dbdes2_packet) : sizeof(struct
						       ospf_dbdes3_packet);
}
...
static void ospf_prepare_dbdes(struct ospf_proto *p, struct ospf_neighbor *n)
{
...
static void ospf_do_send_dbdes(struct ospf_proto *p, struct ospf_neighbor *n)
{
	struct ospf_iface *ifa = n->ifa;
 ...
 * When I send a packet to a struct neighbor I keep a copy in a buffer. If the neighbor
...
static int
ospf_process_dbdes(struct ospf_proto *p, struct ospf_packet *pkt,
...
drop:
	LOG_LSA1("Bad LSA (Type: %04x, Id: %R, Rt: %R) in DBDES", lsa_type,
		 lsa.id, lsa.rt);
	LOG_LSA2("  received from nbr %R on %s - %s", n->rid, ifa->ifname,
		 err_dsc);
		   ...
		   struct ospf_neighbor *n)
{
	...
	if (plen < ospf_dbdes_hdrlen(p)) {
		LOG_PKT("Bad DBDES packet from nbr %R on %s - %s (%u)", n->rid,
			ifa->ifname, "too short", plen);
		return;
	}

	OSPF_PACKET(ospf_dump_dbdes, pkt,
		    "DBDES packet received from nbr %R on %s", n->rid,
		    ifa->ifname);
	...
	case NEIGHBOR_2WAY:
		OSPF_TRACE(D_PACKETS,
			   "DBDES packet ignored - lesser state than ExStart");
		return;
	...
	case NEIGHBOR_EXSTART:
		if ((ifa->type != OSPF_IT_VLINK) &&
			...
			LOG_PKT_WARN
			    ("MTU mismatch with nbr %R on %s (remote %d, local %d)",
			     n->rid, ifa->ifname, rcv_iface_mtu,
			     ifa->iface->mtu);

		if ((rcv_imms == DBDES_IMMS) &&
		    (n->rid > p->router_id) && (plen == ospf_dbdes_hdrlen(p))) {
			/* I'm slave! */
			n->dds = rcv_ddseq;
			n->ddr = rcv_ddseq;
		...
		if (!(rcv_imms & DBDES_I) &&
		    !(rcv_imms & DBDES_MS) &&
		    (n->rid < p->router_id) && (n->dds == rcv_ddseq)) {
			/* I'm master! */
			n->options = rcv_options;
	...
	case NEIGHBOR_EXCHANGE:
		if ((rcv_imms == n->imms) &&
		    (rcv_options == n->options) && (rcv_ddseq == n->ddr))
			goto duplicate;
